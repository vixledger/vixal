...
struct xdr_integral_base : xdr_traits_base {
    using type = T;
    using uint_type = U;
    static constexpr const bool xdr_defined = false;
...
template<typename T, typename U>
struct xdr_fp_base : xdr_traits_base {
    using type = T;
    using uint_type = U;
...
struct xdr_container_base : xdr_traits_base {
    ...
    template<typename Archive>
    static void save(Archive &a, const T &t) {
        if (variable)
...
template<>
struct xdr_struct_base<> : xdr_traits_base {
...
struct field_constructor_t {
    ...
    template<typename T, typename F>
    void operator()(F T::*mp, T &t) const {
        new(&(t.*mp)) F{};
    }

    template<typename T, typename F, typename TT>
    void
...
struct field_archiver_t {
    ...
    template<typename F, typename T, typename Archive>
    void

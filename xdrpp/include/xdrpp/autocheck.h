...
/**
 * \file autocheck.h Support for using the [autocheck framework](https://github.com/thejohnfreeman/autocheck/wiki) with
 * XDR data types.  If you include this file (which requires the autocheck headers to be available), you can generate
 * arbitrary instances of XDR types for testing your application.  For example:
 * \code
 *   // g will generate arbitrary values of type my_xdr_type
 *   autocheck::generator<my_xdr_type> g;
 *   // For variable size objects (vectors, strings, linked lists), size indicates how big to make them.
 *   // For numeric values, size affects how big the number is likely to be.
 *   size_t size = 50;
...
/// If defined to 0, changes the behavior of the library to generate strings with ASCII characters with small size
/// parameters.  The default (1) generates strings with arbitrary bytes.
#define XDR_AUTOCHECK_FUZZY_STRINGS 1
#endif // !XDR_AUTOCHECK_FUZZY_STRINGS

namespace detail {

//! Convert an int to a uint8_t or char, extracting the low byte. This is harder than it sounds when char is signed,
//! because converting to a smaller signed type yields an implementation-defined value.  Worse, MSVC with certain
//! options goes even further and crashes the program when you simply cast an int to a char.  Since there's no
//! equivalent of reinterpret_cast between signed and unsigned values, and since even the standard union trick may be
//! unsafe according to:
//!
//!     http://stackoverflow.com/questions/11373203/accessing-inactive-union-member-undefined
//!
//! we just gave up and did the brute force arithmetic thing, taking advantage of the fact that all these one-byte
//! values get promoted to int for arithmetic.
template<typename T>
constexpr T
to_int8(uint8_t u) {

...
struct rpc_decl {
    string id;
    enum {
        SCALAR, PTR, ARRAY, VEC
    } qual{SCALAR};
    string bound;

    enum {
        TS_ID, TS_ENUM, TS_STRUCT, TS_UNION
    } ts_which{TS_ID};
    string type;
    union {
...
struct rpc_sym {
    ...
    enum symtype {
        CONST, STRUCT, UNION, ENUM, TYPEDEF, PROGRAM, LITERAL,
        NAMESPACE, CLOSEBRACE
    } type;
...
struct indenter : omanip<indenter> {
    ...
    void do_open(ostream &os) {
        ++*this;
        do_indent(os);
    }

    void do_close(ostream &os) {
        --*this;
        do_indent(os);
    }

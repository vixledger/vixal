...
class Config : public std::enable_shared_from_this<Config> {
    ...
    void loadQset(std::shared_ptr<cpptoml::toml_group> group, SCPQuorumSet &qset, int level);

    void parseNodeID(std::string configStr, PublicKey &retKey);

    void parseNodeID(std::string configStr, PublicKey &retKey, SecretKey &sKey, bool isSeed);
...
public:
    ...
    // The default way vixal-core starts is to load the state from disk and catch

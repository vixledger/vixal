...
public:
    ...
    public:
        ...
        explicit InputIterator(std::shared_ptr<Bucket const> bucket);
    ...
    /// Create an empty bucket. The empty bucket has hash '000000...' and its filename is the empty string.
    Bucket();

    /// Destroy a bucket, deleting its underlying file if the bucket is not 'retained'. See `setRetain`.
    ~Bucket();

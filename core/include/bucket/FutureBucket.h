...
/**
 * FutureBucket is a minor wrapper around std::shared_future<std::shared_ptr<Bucket>>, used in merging multiple buckets
 * together in the BucketList. The reason this is a separate class is that we need to support a level of persistence:
 * serializing merges-in-progress in a symbolic fashion, including restarting the merges after we deserialize.
 *
 * This class is therefore used not _only_ in the BucketList but also in places that serialize and deserialize
 * snapshots of it in the form of HistoryArchiveStates: the LedgerManager, when storing kHistoryArchiveState at
 * the bottom of closeLedger; and the HistoryManager, when storing and retrieving HistoryArchiveStates.
 */
class FutureBucket {
    /**
     * There are two lifecycles of a FutureBucket:
     *
     * In one, it's created live, snapshotted at some point in the process (either before or after the merge completes),
     * and resolved/cleared.
     *
     * In another, it's created, deserialized (filling in the hashes of either the inputs or the output, but not both),
     * made-live, and resolved/cleared.
     **/
    ...
    /// These live values hold the input buckets and/or output std::shared_future for a "live" bucket merge in progress.
    /// They will be empty when the FutureBucket is constructed, when it is reset, or when it is freshly deserialized
    /// and not yet activated. When they are nonempty, they should have values equal to the subsequent mFooHash values
    /// below.
    ...
    /// These strings hold the serializable (or deserialized) bucket hashes of the inputs and outputs of a merge;
    /// depending on the state of the FutureBucket they may be empty strings, but if they are nonempty and the
    /// live values (mInputSnapBucket etc.) are also nonempty, they should agree on the hash-values.

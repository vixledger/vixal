...
public:
    explicit StatementContext(std::shared_ptr<soci::statement> stmt) : mStmt(stmt) {
        mStmt->bind_clean_up();
    }
...
/**
 * Object that owns the database connection(s) that an application uses to store the current ledger and other
 * persistent state in.
 *
 * This may represent an in-memory SQLite instance (for testing), an on-disk SQLite instance (for running a minimal,
 * self-contained server) or a connection to a local Postgresql database, that the node operator must have set up
 * on their own.
 *
 * Database connects, on construction, to the target specified by the application Config object's Config::DATABASE
 * value; this originates from the config-file's DATABASE string. The default is "sqlite3://:memory:". This
 * "main connection" is where most SQL statements -- and all write-statements -- are executed.
 *
 * Database may establish additional connections for worker threads to read data, from a separate connection pool,
 * if worker threads request them. The pool will connect to the same target and only one connection will be made per
 * worker thread.
 *
 * All database connections and transactions are set to snapshot isolation level (SQL isolation level 'SERIALIZABLE'
 * in Postgresql and Sqlite, neither of which provide true serializability).
 */
...
public:
    ...
    // by the --newdb command-line flag on VIXAL-core.
    void initialize();
    ...
    using EntryCache = cache::lru_cache<std::string, std::shared_ptr<LedgerEntry const>>;
...
public:
    explicit DBTimeExcluder(Application &mApp);

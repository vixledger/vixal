 ...
 *
 * Drives the SCP consensus protocol, is responsible for collecting Txs and TxSets from the network and making sure
 * Txs aren't lost in ledger close
 *
 * LATER: These interfaces need cleaning up. We need to work out how to make the bidirectional interfaces
 */
class Herder {
public:
    ...
    virtual bool recvSCPQuorumSet(Hash const &hash, SCPQuorumSet const &qset) = 0;

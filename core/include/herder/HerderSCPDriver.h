...
public:
    struct ConsensusData {
        ...
        VixalValue mConsensusValue;

        ConsensusData(uint64_t index, VixalValue const &b)
    ...
    void restoreSCPState(uint64_t index, VixalValue const &value);
    ...
    void nominate(uint64_t slotIndex, VixalValue const &value,
                  TxSetFramePtr proposedSet, VixalValue const &previousValue);
...
private:
    ...
    struct SCPMetrics {
        ...
        explicit SCPMetrics(Application &app);
    };
    ...
    validateValueHelper(uint64_t slotIndex, VixalValue const &sv) const;

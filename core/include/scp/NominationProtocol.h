...
using ValueProcessor = std::function<void(Value const &val)>;
...
class NominationProtocol {
    ...
    std::unique_ptr<SCPEnvelope> mLastEnvelope; // last envelope emitted by this node
    ...
    static bool isNewerStatement(SCPNomination const &oldst, SCPNomination const &st);

    // returns true if 'p' is a subset of 'v'
    // also sets 'notEqual' if p and v differ
    // note: p and v must be sorted
    static bool isSubsetHelper(xdr::xvector<Value> const &p, xdr::xvector<Value> const &v, bool &notEqual);
    ...
    static void applyAll(SCPNomination const &nom, ValueProcessor processor);
...
public:
    explicit NominationProtocol(Slot &slot);
    ...
    bool nominate(Value const &value, Value const &previousValue, bool timedout);

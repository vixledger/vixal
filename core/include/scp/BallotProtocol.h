...
using StatementPredicate = std::function<bool(SCPStatement const &st)>;
...
public:
    explicit BallotProtocol(Slot &slot);
    ...
    // returns the hash of the QuorumSet that should be downloaded with the statement.
    // note: the companion hash for an EXTERNALIZE statement does not match the hash of the QSet,
    // but the hash of commitQuorumSetHash
    static Hash getCompanionQuorumSetHashFromStatement(SCPStatement const &st);

    // helper function to retrieve b for PREPARE, P for CONFIRM or c for EXTERNALIZE messages

...
class ManageOfferOpFrame : public OperationFrame {
    ...
    bool checkOfferValid(medida::MetricsRegistry &metrics, Database &db, LedgerDelta &delta);
    ...
    OfferEntry buildOffer(AccountID const &account, ManageOfferOp const &op, uint32 flags);

protected:
    bool mPassive;

public:
    ManageOfferOpFrame(Operation const &op, OperationResult &res, TransactionFrame &parentTx);

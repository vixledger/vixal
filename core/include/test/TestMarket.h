...
struct OfferState {
    ...
    OfferState()
            : selling{txtest::makeNativeAsset()}, buying{txtest::makeNativeAsset()}, price{Price{1, 1}}, amount{-1} {
    }

    OfferState(Asset selling, Asset buying, Price price, int64_t amount,
               OfferType type = OfferType::ACTIVE)
            : selling{std::move(selling)}, buying{std::move(buying)}, price{std::move(price)}, amount{amount},
              type{type} {
    }

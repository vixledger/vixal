...
class DataFrame : public EntryFrame {
    ...
    static void loadData(StatementContext &prep, LedgerEntryProcessor dataProcessor);
...
public:
    ...
    vixal::DataValue const &getValue() const;
    ...
    static uint64_t countObjects(soci::session &sess, LedgerRange const &ledgers);

    static void deleteDataModifiedOnOrAfterLedger(Database &db, uint32_t oldestLedger);

...
public:
    ...
    // returned.
    //
    // Otherwise LedgerManager returns VERIFY_STATUS_OK if the proposed ledger is
    // a first member of that buffer (and has matching hash).
    ...
    verifyCatchupCandidate(LedgerHeaderHistoryEntry const &candidate, bool manualCatchup) const = 0;

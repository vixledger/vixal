...
/**
 * OverlayManager maintains a virtual broadcast network, consisting of a set of remote TCP peers (TCPPeer),
 * a mechanism for flooding messages to all peers (FloodGate), and a mechanism for sending and receiving anycast
 * request/reply pairs (ItemFetcher).
 *
 * Overlay network messages are defined as the XDR structure type `VixalMessage`, in the file
 * src/xdr/Stellar-overlay.x
 *
 * They are minimally framed using the Record Marking (RM) standard of RFC5531
 * (https://tools.ietf.org/html/rfc5531#page-16) and the RM-framed messages are transmitted over TCP/IP sockets,
 * between peers.
 *
 * The `VixalMessage` union contains 3 logically distinct kinds of message:
 *
 ...
 *
 * Anycasts are initiated and serviced two instances of ItemFetcher (mTxSetFetcher and mQuorumSetFetcher).
 * Anycast messages are sent to directly-connected peers, in sequence until satisfied. They are not
 * flooded between peers.
 *
 * Broadcasts are initiated by the Herder and sent to both the Herder _and_ the local FloodGate, for propagation
 * to other peers.
 *
 * The OverlayManager tracks its known peers in the Database and shares peer records with other peers when asked.
 */
namespace vixal {
...
public:
    ...
    // Send a given message to all peers, via the FloodGate. This is called by Herder.
    virtual void broadcastMessage(VixalMessage const &msg, bool force) = 0;
    ...
    virtual void recvFloodedMsg(VixalMessage const &msg, Peer::pointer peer) = 0;
    ...
    virtual Peer::pointer getConnectedPeer(std::string const &ip, unsigned short port) = 0;

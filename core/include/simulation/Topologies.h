...
public:
    ...
    static Simulation::pointer core(int nNodes, double quorumThresholdFraction,
                                    Simulation::Mode mode,
                                    Hash const &networkID,
                                    std::function<Config()> confGen = nullptr);

    // nNodes with same qSet - one way connection in cycle
    static Simulation::pointer cycle(int nNodes, double quorumThresholdFraction,
                                     Simulation::Mode mode,
                                     Hash const &networkID,
                                     std::function<Config()> confGen = nullptr);
    ...
    static Simulation::pointer
    branchedcycle(int nNodes, double quorumThresholdFraction,
                  Simulation::Mode mode, Hash const &networkID,
                  std::function<Config()> confGen = nullptr);

    // nNodes with same qSet - no connection created
    static Simulation::pointer
    separate(int nNodes, double quorumThresholdFraction, Simulation::Mode mode,
             Hash const &networkID, std::function<Config()> confGen = nullptr);

    // multi-tier quorum (core4 + mid-tier nodes that depend on 2 nodes of core4)
    // mid-tier connected round-robin to core4
    static Simulation::pointer
    hierarchicalQuorum(int nBranches, Simulation::Mode mode,
                       Hash const &networkID,
                       std::function<Config()> confGen = nullptr, int connectionsToCore = 1);

    // 2-tier quorum with a variable size core (with 0.75 threshold) and outer-nodes that listen to core & self
    // outer-nodes have connectionsToCore connections to core nodes (round-robin)
    static Simulation::pointer

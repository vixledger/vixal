...
private:
    ...
    recvSCPEnvelope(SCPEnvelope const &envelope) override {
        received.emplace_back(envelope.statement.pledges.confirm().nPrepared);
...
makeEnvelope(int id) {
    ...
    result.statement.slotIndex = static_cast<uint64>(++slotIndex);
    result.statement.pledges.type(SCP_ST_CONFIRM);
    result.statement.pledges.confirm().nPrepared = static_cast<uint32>(id);
    return result;
}
}

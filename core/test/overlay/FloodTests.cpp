...
TEST_CASE("Flooding", "[flood][overlay]") {
    ...
    auto test = [&](std::function<void(int)> inject,
        ...
        auto rootA = AccountFrame::loadAccount(root.getPublicKey(), app0->getDatabase());
        ...
        for (auto n : nodes) {
            ...
            for (auto const &kv : m.getAllMetrics()) {
                ...
                if (metric.domain() == "overlay") {
                    out << metric.domain() << "." << metric.type() << "." << metric.name() << std::endl;
    ...
    SECTION("transaction flooding") {
        auto injectTransaction = [&](int i) {
            ...
            auto tx1 = account.tx({createAccount(dest.getPublicKey(), txAmount)}, expectedSeq);

            // this is basically a modified version of Peer::recvTransaction
            auto msg = tx1->toVixalMessage();
        ...
        auto ackedTransactions = [&](std::shared_ptr<Application> app) {
            ...
            for (auto const &s : sources) {
                okCount += (app->getHerder().getMaxSeqInPendingTxs(s) == expectedSeq) ? 1 : 0;
            }
        ...
        SECTION("outer nodes") {
            SECTION("loopback") {
                simulation =
                        Topologies::hierarchicalQuorumSimplified(5, 10, Simulation::OVER_LOOPBACK, networkID, cfgGen);
                test(injectTransaction, ackedTransactions);
            }
            SECTION("tcp") {
    ...
    SECTION("scp messages flooding") {
        ...
        auto injectSCP = [&](int i) {
            ...
            VixalValue sv(txSet.getContentsHash(), lcl.header.scpValue.closeTime + 1, emptyUpgradeSteps, 0);
        ...
        SECTION("core") {
            SECTION("loopback") {
                simulation = Topologies::core(4, .666f, Simulation::OVER_LOOPBACK,
                                              networkID, cfgGen, quorumAdjuster);
                test(injectSCP, ackedSCP);
            }
            SECTION("tcp") {
                simulation = Topologies::core(4, .666f, Simulation::OVER_TCP, networkID,

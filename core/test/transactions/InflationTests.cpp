...
createTestAccounts(Application &app, int nbAccounts,
    ...
    for (int i = 0; i < nbAccounts; i++) {
        ...
        if (bal >= 0) {
            ...
            act->getAccount().inflationDest.activate() = getTestAccount(getVote(i)).getPublicKey();
...
TEST_CASE("inflation", "[tx][inflation]") {
    ...
    SECTION("not time") {
        for_all_versions(*app, [&] {
            ...
            REQUIRE(app->getLedgerManager().getCurrentLedgerHeader().inflationSeq == 0);
            ...
            REQUIRE(app->getLedgerManager().getCurrentLedgerHeader().inflationSeq == 1);
            ...
            root.inflation();
            REQUIRE(app->getLedgerManager().getCurrentLedgerHeader().inflationSeq == 2);
            ...
            root.inflation();
            REQUIRE(app->getLedgerManager().getCurrentLedgerHeader().inflationSeq == 3);
    ...
    SECTION("total coins") {
        ...
        REQUIRE(beforeInflationRoot + beforeInflationVoter1 + beforeInflationVoter2 + beforeInflationTarget1 +
                beforeInflationTarget2 + clh.feePool == clh.totalCoins);
        ...
        for_versions_to(7, *app, [&] {
            ...
            REQUIRE(afterInflationRoot + afterInflationVoter1 + afterInflationVoter2 + afterInflationTarget1 +
                    afterInflationTarget2 + clh.feePool == clh.totalCoins + inflationError);
        });
        ...
        for_versions_from(8, *app, [&] {
            ...
            REQUIRE(afterInflationRoot + afterInflationVoter1 + afterInflationVoter2 + afterInflationTarget1 +
                    afterInflationTarget2 + clh.feePool == clh.totalCoins);
        });
    }
    ...
    const int64 winnerVote =
            bigDivide(app->getLedgerManager().getCurrentLedgerHeader().totalCoins, 5, 10000, ROUND_DOWN);

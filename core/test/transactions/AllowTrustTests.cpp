...
TEST_CASE("allow trust", "[tx][allowtrust]") {
    ...
    SECTION("allow trust not required") {
        for_all_versions(*app, [&] {
            REQUIRE_THROWS_AS(gateway.allowTrust(idr, a1), ex_ALLOW_TRUST_TRUST_NOT_REQUIRED);
            REQUIRE_THROWS_AS(gateway.denyTrust(idr, a1), ex_ALLOW_TRUST_TRUST_NOT_REQUIRED);
        });
    }
    ...
    SECTION("self allow trust") {
        SECTION("allow trust with trustline") {
            for_versions_to(2, *app, [&] {
                REQUIRE_THROWS_AS(gateway.allowTrust(idr, gateway), ex_ALLOW_TRUST_TRUST_NOT_REQUIRED);
                REQUIRE_THROWS_AS(gateway.denyTrust(idr, gateway), ex_ALLOW_TRUST_TRUST_NOT_REQUIRED);
            });

            for_versions_from(3, *app, [&] {
                REQUIRE_THROWS_AS(gateway.allowTrust(idr, gateway), ex_ALLOW_TRUST_SELF_NOT_ALLOWED);
                REQUIRE_THROWS_AS(gateway.denyTrust(idr, gateway), ex_ALLOW_TRUST_SELF_NOT_ALLOWED);
            });
        }
        ...
        SECTION("allow trust without explicit trustline") {
            ...
            SECTION("do not set revocable flag") {
                for_versions_to(2, *app, [&] {
                    ...
                    REQUIRE_THROWS_AS(gateway.denyTrust(idr, gateway), ex_ALLOW_TRUST_CANT_REVOKE);
                });

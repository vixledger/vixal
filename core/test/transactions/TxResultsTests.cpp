...
struct ExpectedOpResult {
    ...
    ExpectedOpResult(SetOptionsResultCode setOptionsResultCode)
            : code{opINNER}, type{SET_OPTIONS}, setOptionsResultCode{setOptionsResultCode} {
    }
};
...
TEST_CASE("txresults", "[tx][txresults]") {
    ...
    auto validate = [&](TransactionFramePtr const &tx, ValidationResult validationResult,
    ...
    auto makeApplyResult = [&](std::vector<Signed> const &opSigned,
        ...
        for (auto i = 0; i < opPayment.size(); i++) {
            ...
            switch (opPayment[i]) {
                case PaymentValidity::UNDERFUNDED:
                    if (firstUnderfunded) {
                        opResults.emplace_back(PAYMENT_UNDERFUNDED);
                        firstUnderfunded = false;
                    } else {
                        opResults.emplace_back(PAYMENT_SUCCESS);
                    }
                    break;
                case PaymentValidity::VALID:
                    opResults.emplace_back(PAYMENT_SUCCESS);
    ...
    for (const auto &signs : sign3) {
        SECTION(signSectionName(signs)) {
            for (const auto &ops : op3) {
    ...
    SECTION("5 signed but underfunded") {
        test({Signed::SIGNED, Signed::SIGNED,
    ...
    SECTION("not enough signature weight") {
        ...
        th.masterWeight = make_optional(10u);
        th.lowThreshold = make_optional(10u);
        th.medThreshold = make_optional(50u);
        th.highThreshold = make_optional(100u);

        SECTION("normal") {
            auto tx = a.tx({payment(b, 1000), setOptions(nullptr, nullptr, nullptr, &th, nullptr, nullptr)});
            for_all_versions(*app, [&] {
                validate(
                        tx, {baseFee * 2, txSUCCESS},
                        expectedResult(baseFee * 2, 2, txSUCCESS,

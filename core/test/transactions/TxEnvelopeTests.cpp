            ...
            SECTION(alternative.name) {
                ...
                SECTION("single signature") {
                    ...
                    SECTION("failing transaction") {
                        ...
                        for_versions(3, 9, *app, [&] {
                            applyCheck(tx, *app);
                            REQUIRE(tx->getResultCode() == vixal::txFAILED);
                            REQUIRE(PaymentOpFrame::getInnerCode(getFirstResult(
                                    *tx)) == vixal::PAYMENT_MALFORMED);
                            REQUIRE(getAccountSigners(a1, *app).size() == 1);
                        });
                        for_versions_from(10, *app, [&] {
                            applyCheck(tx, *app);
            ...
            SECTION("multiple tx") {
                for_all_versions(*app, [&] {
                    ...
                    SECTION("one failed tx") {
                        ...
                        tx_a->getEnvelope().tx.operations.push_back(tx_b->getEnvelope().tx.operations[0]);
                        tx_a->getEnvelope().tx.fee *= 2;
                        TransactionFramePtr tx = TransactionFrame::makeTransactionFromWire(
                                app->getNetworkID(), tx_a->getEnvelope());
                    ...
                    SECTION("both success") {
                        ...
                        tx_b->getEnvelope().tx.operations[0].sourceAccount.
                                activate() = b1.getPublicKey();
...

        {
            SECTION("Insufficient fee") {
                for_all_versions(*app, [&] {
                    ...
                    txFrame->getEnvelope().tx.fee = static_cast<uint32_t>( app->getLedgerManager().getTxFee() - 1);

        ...
        SECTION(sectionName) {
            ...
            if (range.second) {
                ...
                if (std::numeric_limits<uint32_t>::max() -
                    historyManager.getCheckpointFrequency() >= configuration.count()) {
                    // but at most count + getCheckpointFrequency
                    // doing more would mean we are doing non-needed work
                    REQUIRE(appliedCount <= configuration.count() + historyManager.getCheckpointFrequency());
                }
            } else {
                ...
                REQUIRE(lastClosedLedger + historyManager.getCheckpointFrequency() >= checkpointRange.first());
                if (std::numeric_limits<uint32_t>::max() -

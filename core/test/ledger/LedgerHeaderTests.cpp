...
TEST_CASE("genesisledger", "[ledger]") {
    ...
    REQUIRE(binToHex(header.bucketListHash) == "4e6a8404d33b17eee7031af0b3606b6af8e36fe5a3bff59e4e5e420bd0ad3bf4");
...
TEST_CASE("ledgerheader", "[ledger]") {
    ...
    {
        VirtualClock clock;
        Application::pointer app = Application::create<ApplicationImpl>(clock, cfg);
        app->start();
        ...
        // close this ledger
        VixalValue sv(txSet->getContentsHash(), 1, emptyUpgradeSteps, 0);
    ...
    SECTION("load existing ledger") {
        ...
        VirtualClock clock2;
        Application::pointer app2 = Application::create<ApplicationImpl>(clock2, cfg2, false);
        app2->start();

        REQUIRE(saved == app2->getLedgerManager().getLastClosedLedgerHeader().hash);
    }
}

...
TEST_CASE("encoded_size32", "[decoder]") {
    for (auto const &item : b32_data) {
        SECTION(item.first) {
            REQUIRE(item.second.size() == decoder::encoded_size32(item.first.size()));
        }
    }
}
...
TEST_CASE("encoded_size64", "[decoder]") {
    for (auto const &item : b64_data) {
        SECTION(item.first) {
            REQUIRE(item.second.size() == decoder::encoded_size64(item.first.size()));
        }
    }
}
...
TEST_CASE("decode_b64 with iterators", "[decoder]") {
    for (auto const &item : b64_data) {
        SECTION(item.second) {
            ...
            decoder::decode_b64(std::begin(item.second), std::end(item.second), std::back_inserter(out));
            REQUIRE(item.first == out);
        }
    }
}

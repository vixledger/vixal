...
TEST_CASE("Full history catchup", "[history][historycatchup]") {
    ...
    uint32_t initLedger = catchupSimulation.getApp().getLedgerManager().getLastClosedLedgerNum();

    std::vector<Application::pointer> apps;

    std::vector<uint32_t> counts = {0, std::numeric_limits<uint32_t>::max(), 60};

    std::vector<Config::TestDbMode> dbModes = {Config::TESTDB_IN_MEMORY_SQLITE, Config::TESTDB_ON_DISK_SQLITE};
#ifdef USE_POSTGRES
    if (!force_sqlite)
    ...
    for (auto dbMode : dbModes) {
        for (auto count : counts) {
            auto a = catchupSimulation.catchupNewApplication(
                    ...
                    std::string("full, ") + resumeModeName(count) + ", " + dbModeName(dbMode));
            apps.push_back(a);
        }
    }
}
...
TEST_CASE("History publish queueing", "[history][historydelay][historycatchup]") {
    ...
    auto initLedger = catchupSimulation.getApp().getLedgerManager().getLastClosedLedgerNum();
    auto app2 = catchupSimulation.catchupNewApplication(initLedger, std::numeric_limits<uint32_t>::max(), false,
                                                        Config::TESTDB_IN_MEMORY_SQLITE,
                                                        std::string("Catchup to delayed history"));
    CHECK(app2->getLedgerManager().getLedgerNum() == catchupSimulation.getApp().getLedgerManager().getLedgerNum());
}
...
TEST_CASE("Publish/catchup alternation, with stall",
    ...
    app3 = catchupSimulation.catchupNewApplication(initLedger, 0, false,
                                                   Config::TESTDB_IN_MEMORY_SQLITE,
                                                   std::string("app3"));
    ...
    for (size_t i = 1; i < 4; ++i) {
        ...
        catchupSimulation.catchupApplication(initLedger, std::numeric_limits<uint32_t>::max(), false, app2);
    ...
    REQUIRE(catchupSimulation.catchupApplication(initLedger, 0, false, app3, false));
}
...
TEST_CASE("Repair missing buckets via history",
    ...
    HistoryArchiveState has(catchupSimulation.getApp().getLedgerManager().getLastClosedLedgerNum(),
    ...
    auto hash1 = catchupSimulation.getApp().getBucketManager().getBucketList().getHash();
...
TEST_CASE("Publish/catchup via s3", "[hide][s3]") {
    ...
    auto app2 = catchupSimulation.catchupNewApplication(
            catchupSimulation.getApp().getLedgerManager().getCurrentLedgerHeader().ledgerSeq,
...
TEST_CASE("too far behind / catchup restart", "[history][catchupstall]") {
    ...
    init = catchupSimulation.getApp().getLedgerManager().getLastClosedLedgerNum();
...
TEST_CASE("Catchup recent", "[history][catchuprecent]") {
    ...
    for (auto r : recents) {
        ...
        apps.push_back(catchupSimulation.catchupNewApplication(initLedger, r, false, dbMode, name));
    }

    // Now push network along a little bit and see that they can all still
    // catch up properly.
    catchupSimulation.generateAndPublishHistory(2);
    initLedger = catchupSimulation.getApp().getLedgerManager().getLastClosedLedgerNum();
...
TEST_CASE("Catchup manual", "[history][catchupmanual]") {
    ...
    for (auto const &test : vixal::gCatchupRangeCases) {
        ...
        auto a = catchupSimulation.catchupNewApplication(configuration.toLedger(),
                                                         configuration.count(), true, dbMode, name);
        // manual catchup-complete
        catchupSimulation.catchupApplication(initLedger, std::numeric_limits<uint32_t>::max(),
                                             true, a);
        apps.push_back(a);
    }

...
#include "../../src/bucket/BucketManagerImpl.h"
...
TEST_CASE("skip list", "[bucket]") {
    ...
    public:
        BucketManagerTest(Application &a) : BucketManagerImpl(a) {
        }
...
TEST_CASE("bucket list", "[bucket]") {
    ...
    try {
        ...
        for (uint32_t i = 1; !app->getClock().io_context().stopped() && i < 130; ++i) {
            app->getClock().crank(false);
            bl.addBatch(*app, i, LedgerTestUtils::generateValidLedgerEntries(8), deadGen(5));
            if (i % 10 == 0) {
                CLOG(DEBUG, "Bucket") << "Added batch " << i << ", hash=" << binToHex(bl.getHash());
            }
        ...
        CLOG(DEBUG, "Bucket") << "Test caught std::future_error " << e.code() << ": " << e.what();
        REQUIRE(false);
    }
}
...
TEST_CASE("bucket list shadowing", "[bucket]") {
    ...
    for (uint32_t i = 1; !app->getClock().io_context().stopped() && i < 1200; ++i) {
        app->getClock().crank(false);
...
TEST_CASE("duplicate bucket entries", "[bucket]") {
    ...
    try {
        ...
        for (uint32_t i = 1; !app->getClock().io_context().stopped() && i < 130; ++i) {
            ...
            if (i % 10 == 0) {
                CLOG(DEBUG, "Bucket") << "Added batch " << i << ", hash1=" << hexAbbrev(bl1.getHash())
...
TEST_CASE("bucket tombstones expire at bottom level", "[bucket][tombstones]") {
    ...
    for (uint32_t i = 0; i < BucketList::kNumLevels; ++i) {
        ...
        level.setCurr(Bucket::fresh(bm, LedgerTestUtils::generateValidLedgerEntries(8), deadGen(8)));
        level.setSnap(Bucket::fresh(bm, LedgerTestUtils::generateValidLedgerEntries(8), deadGen(8)));
    }

    for (uint32_t i = 0; i < BucketList::kNumLevels; ++i) {
        std::vector<uint32_t> ledgers = {BucketList::levelHalf(i), BucketList::levelSize(i)};
        for (auto j : ledgers) {
            auto n = mergeTimer.count();
            bl.addBatch(*app, j, LedgerTestUtils::generateValidLedgerEntries(8), deadGen(8));
            app->getClock().crank(false);
            ...
            CLOG(INFO, "Bucket") << "Added batch at ledger " << j << ", merges provoked: " << n;
            REQUIRE(n > 0);
...
TEST_CASE("file-backed buckets", "[bucket][bucketbench]") {
    ...
    std::shared_ptr<Bucket> b1 = Bucket::fresh(app->getBucketManager(), live, dead);
    for (uint32_t i = 0; i < 5; ++i) {
        CLOG(DEBUG, "Bucket") << "Merging 10000 new ledger entries into " << (i * 10000) << " entry bucket";
        for (auto &e : live) {
            e = LedgerTestUtils::generateValidLedgerEntry(3);
        }
        for (auto &e : dead) {
...
TEST_CASE("merging bucket entries", "[bucket]") {
    ...
    SECTION("random dead entries annihilates live entries") {
        ...
        CLOG(DEBUG, "Bucket") << "post-merge live count: " << liveCount << " of " << live.size();
    ...
    SECTION("random live entries overwrite live entries in any order") {
        ...
        std::shared_ptr<Bucket> b2 = Bucket::fresh(app->getBucketManager(), live, dead);
        std::shared_ptr<Bucket> b3 = Bucket::merge(app->getBucketManager(), b1, b2);
...
TEST_CASE("bucketmanager ownership", "[bucket]") {
    ...
    std::vector<LedgerEntry> live(LedgerTestUtils::generateValidLedgerEntries(10));
    std::vector<LedgerKey> dead{};
...

    {
        ...
        std::shared_ptr<Bucket> b3 = Bucket::fresh(app->getBucketManager(), live, dead);
        std::shared_ptr<Bucket> b4 = Bucket::fresh(app->getBucketManager(), live, dead);
...
TEST_CASE("single entry bubbling up", "[bucket][bucketbubble]") {
    ...
    try {
        ...
        std::vector<vixal::LedgerEntry> emptySetEntry;

        CLOG(DEBUG, "Bucket") << "Adding single entry in lowest level";
        bl.addBatch(*app, 1, LedgerTestUtils::generateValidLedgerEntries(1), emptySet);

        CLOG(DEBUG, "Bucket") << "Adding empty batches to bucket list";
        for (uint32_t i = 2; !app->getClock().io_context().stopped() && i < 300; ++i) {
            app->getClock().crank(false);
            ...
            for (uint32_t j = 0; j <= BucketList::kNumLevels - 1; ++j) {
                ...
                auto currSz = countEntries(lev.getCurr());
                auto snapSz = countEntries(lev.getSnap());
                CLOG(DEBUG, "Bucket") << "ledger " << i << ", level " << j << " curr=" << currSz << " snap=" << snapSz;
                if (1 > lb && 1 <= hb) {
...
closeLedger(Application &app) {
    ...
    CLOG(INFO, "Bucket") << "Artificially closing ledger " << lm.getLedgerNum()
    ...
    VixalValue sv(txSet->getContentsHash(), lm.getCloseTime(), emptyUpgradeSteps, 0);
...
TEST_CASE("bucket persistence over app restart", "[bucket][bucketpersist]") {
    ...
    cfg1.FORCE_SCP = false;
    {
        ...
        REQUIRE(hexAbbrev(Lh1) == hexAbbrev(app->getLedgerManager().getLastClosedLedgerHeader().hash));
        REQUIRE(hexAbbrev(Blh1) == hexAbbrev(bl.getHash()));
...
TEST_CASE("checkdb succeeding", "[bucket][checkdb]") {
    ...
    SECTION("failing checkdb") {
        app->checkDB();
        app->getDatabase().getSession() << ("UPDATE accounts SET balance = balance * 2"
...
TEST_CASE("bucket apply", "[bucket]") {
    ...
    std::shared_ptr<Bucket> birth = Bucket::fresh(app->getBucketManager(), live, noDead);

    std::shared_ptr<Bucket> death = Bucket::fresh(app->getBucketManager(), noLive, dead);

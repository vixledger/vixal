...
checkBucketListIsConsistentWithDatabase(
        std::function<std::vector<EntryFrame::pointer>(uint32_t, LedgerEntryType,
                                                       T &)> genLive = generateValidEntryFrames,
        std::function<std::vector<LedgerKey>(uint32_t, std::set<LedgerKey> const &, std::default_random_engine &,
                                             T &)> genDead = {}) {
    std::default_random_engine gen;
...
public:
    ApplyBucketsWorkAddEntry(
            ...
            : ApplyBucketsWork(app, parent, buckets, applyState), mFromLedgerSeq{addAtLedgerSeq}, mType{addType},
              mAdded{false} {
...
public:
    ApplyBucketsWorkDeleteEntry(
            ...
            LedgerEntryType)
            : ApplyBucketsWork(app, parent, buckets, applyState), mKey(target.first), mFrame(target.second),
              mDeleted{false} {
    }
    ...
    for (uint32_t level = 0; level < BucketList::kNumLevels; ++level) {
        ...
        for (uint32_t i = 0; i < 10; ++i) {
            ...
            for (auto target : {lowTarget, highTarget}) {
                ...
                REQUIRE_THROWS_AS(applyBucketsAndCrankUntilDone(appGenerate, appApply, ledgerSeq),
                                  InvariantDoesNotHold);
            }
        }
    }
...
TEST_CASE("BucketListIsConsistentWithDatabase merged LIVEENTRY and DEADENTRY",
    ...
    for (auto t : xdr::xdr_traits<LedgerEntryType>::enum_values()) {
        ...
        while (nTests < 5) {
            ...
            VirtualClock clock;
            Application::pointer appGenerate = Application::create(clock, getTestConfig(0));
            Application::pointer appApply = Application::create(clock, getTestConfig(1));
            auto &blGenerate = appGenerate->getBucketManager().getBucketList();
            ...
            uint32_t ledgerSeq = generateLedgers(
                    appGenerate, 2, 10, 5,
                    std::bind(generateValidEntryFramesAndSelectByType, _1, let, std::ref(selected)), 2,
                    std::bind(generateDeadEntriesAndSelectByType, _1, _2, std::ref(gen), std::ref(selected)));
            if (selected.empty()) {
                continue;
            }
            LedgerEntry entry;
            ...
            REQUIRE(!doesBucketListContain(blGenerate, dead));
            REQUIRE(!doesBucketListContain(blGenerate, live));
            REQUIRE_NOTHROW(applyBucketsAndCrankUntilDone(appGenerate, appApply, ledgerSeq));
            REQUIRE(!EntryFrame::exists(appApply->getDatabase(), key));
            ++nTests;
        }
    }

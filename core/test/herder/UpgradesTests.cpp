...
simulateUpgrade(std::vector<LedgerUpgradeNode> const &nodes,
    ...
    auto statesMatch = [&](std::vector<LedgerUpgradeableData> const &state) {
        for (size_t i = 0; i < nodes.size(); i++) {
            ...
            REQUIRE(node->getLedgerManager().getCurrentLedgerHeader().ledgerVersion == state[i].ledgerVersion);
            REQUIRE(node->getLedgerManager().getCurrentLedgerHeader().baseFee == state[i].baseFee);
            REQUIRE(node->getLedgerManager().getCurrentLedgerHeader().maxTxSetSize == state[i].maxTxSetSize);
            REQUIRE(node->getLedgerManager().getCurrentLedgerHeader().baseReserve == state[i].baseReserve);
        }
    };
...
testListUpgrades(VirtualClock::time_point preferredUpgradeDatetime,
    ...
    header.scpValue.closeTime = static_cast<uint64>(VirtualClock::to_time_t(genesis(0, 0)));
    ...
    auto baseReserveUpgrade = makeBaseReserveUpgrade(cfg.TESTING_UPGRADE_RESERVE);
...
testValidateUpgrades(VirtualClock::time_point preferredUpgradeDatetime,
    ...
    auto checkWith = [&](bool nomination) {
        ...
        SECTION("tx count") {
            ...
            REQUIRE(!Upgrades{cfg}.isValid(checkTime, toUpgradeType(makeTxCountUpgrade(0)),
                                           ledgerUpgradeType, nomination, cfg));
        }
        SECTION("reserve") {
            if (nomination) {
...
TEST_CASE("Ledger Manager applies upgrades properly", "[upgrades]") {
    ...
    auto executeUpgrades = [&](xdr::xvector<UpgradeType, 6> const &upgrades) {
        VixalValue sv{txSet->getContentsHash(), 2, upgrades, 0};
    ...
    SECTION("ledger version") {
        REQUIRE(executeUpgrade(makeProtocolVersionUpgrade(cfg.LEDGER_PROTOCOL_VERSION))
                        .header.ledgerVersion == cfg.LEDGER_PROTOCOL_VERSION);
    }

    SECTION("base fee") {
        REQUIRE(executeUpgrade(makeBaseFeeUpgrade(1000)).header.baseFee == 1000);
    }

    SECTION("max tx") {
        REQUIRE(executeUpgrade(makeTxCountUpgrade(1300)).header.maxTxSetSize == 1300);
    }

    SECTION("base reserve") {
        REQUIRE(executeUpgrade(makeBaseReserveUpgrade(1000)).header.baseReserve == 1000);
    }

    SECTION("all") {
        auto header =
                executeUpgrades({toUpgradeType(makeProtocolVersionUpgrade(cfg.LEDGER_PROTOCOL_VERSION)),

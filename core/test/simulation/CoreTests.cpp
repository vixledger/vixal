...
public:
    ScaleReporter(std::vector<std::string> const &columns)
            : mColumns(columns), mFilename(fmt::format("{:s}-{:d}.csv", join(columns, "-vs-"),
                                                       std::time(nullptr))), mOut(mFilename) {
...
TEST_CASE("Bucket-list entries vs. write throughput", "[scalability][hide]") {
    ...
    for (uint32_t i = 1; !app->getClock().io_context().stopped() && i < 0x200000; ++i) {
        ...
        if ((i & 0xff) == 0xff) {
            r.write({(double) obj.count(), (double) byte.count(),
                     ...
                     batch.snapshot().get99thPercentile(), batch.max(),

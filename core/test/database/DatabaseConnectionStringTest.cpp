...
TEST_CASE("remove password from database connection string",
    ...
    SECTION("password is removed if first") {
        REQUIRE(removePasswordFromConnectionString(
                R"(postgresql://password=abc dbname=vixal)") ==
                R"(postgresql://password=******** dbname=vixal)");
    }

    SECTION("password is removed if second") {
        REQUIRE(removePasswordFromConnectionString(
                R"(postgresql://dbname=vixal password=dbname)") ==
                R"(postgresql://dbname=vixal password=********)");
    }
    ...
    SECTION("quoted password is removed") {
        REQUIRE(
                removePasswordFromConnectionString(
                        R"(postgresql://dbname=vixal password='quoted password')") ==
                R"(postgresql://dbname=vixal password=********)");
    }
    ...
    SECTION("parameters after password remain unchanged") {
        REQUIRE(
                removePasswordFromConnectionString(
                        R"(postgresql://dbname=vixal password='quoted \\ password' performance='as fast as possible')") ==
                R"(postgresql://dbname=vixal password=******** performance='as fast as possible')");
    }

    SECTION("dbname can be quored") {
        REQUIRE(
                removePasswordFromConnectionString(
                        R"(postgresql://dbname='vixal with spaces' password='quoted \\ password' performance='as fast as possible')") ==
                R"(postgresql://dbname='vixal with spaces' password=******** performance='as fast as possible')");
    }

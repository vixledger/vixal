...
checkMVCCIsolation(Application::pointer app) {
...

    {
        ...
        if (app->getDatabase().isSqlite()) {
            ...
            try {
                soci::statement st = sess2.prepare << "UPDATE test SET x=:v";
                soci::use(tx2v1);
                st.execute(true);
                REQUIRE(false);
            }
...
TEST_CASE("postgres smoketest", "[db]") {
    ...
    try {
        ...
        SECTION("blob storage") {
            ...
            session << "insert into test (a, b) values (:aa, :bb)", soci::use(a, "aa"), soci::use(blobX, "bb");

            soci::blob blobY(session);
            session << "select a, b from test", soci::into(b), soci::into(blobY);
            CHECK(blobY.get_len() == x.size());
            y.resize(blobY.get_len());
            blobY.read(0, reinterpret_cast<char *>(y.data()), y.size());
            CHECK(x == y);

            LOG(DEBUG) << "blob round trip with postgresql database: " << binToHex(x) << " == " << binToHex(y);
            tx.commit();
        }

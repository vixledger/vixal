...
BucketManagerImpl::BucketManagerImpl(Application &app)
        : mApp(app), mWorkDir(nullptr), mLockedBucketDir(nullptr), mBucketObjectInsert(
        app.getMetrics().newMeter({"bucket", "object", "insert"}, "object")), mBucketByteInsert(
        app.getMetrics().newMeter({"bucket", "byte", "insert"}, "byte")),
          mBucketAddBatch(app.getMetrics().newTimer({"bucket", "batch", "add"})),
          mBucketSnapMerge(app.getMetrics().newTimer({"bucket", "snap", "merge"})), mSharedBucketsSize(

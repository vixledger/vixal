...
BucketListDepth::BucketListDepth(uint32_t numLevels)
...
BucketList::sizeOfCurr(uint32_t ledger, uint32_t level) {
    ...
    if (level != BucketList::kNumLevels - 1 && mask(ledger, half) != 0) {
        uint32_t const sizeDelta = 1U << (2 * level - 1);
...
BucketList::addBatch(Application &app, uint32_t currLedger,
    ...
    for (uint32_t i = static_cast<uint32_t>(mLevels.size()) - 1; i > 0; --i) {
        assert(shadows.size() >= 2);
        shadows.pop_back();
        shadows.pop_back();
    ...
    mLevels[0].prepare(app, currLedger, Bucket::fresh(app.getBucketManager(), liveEntries, deadEntries),
                       shadows);
    mLevels[0].commit();
}

...
stored in a secondary structure called the [BucketList](../../include/bucket/BucketList.h), which is
composed of a fixed number of "levels", each of which contains two
[Buckets](../../include/bucket/Bucket.h). Each level's Buckets are a multiple of the size of the
...
which is stored in the [ledger header](../../include/xdr/ledger.x) in order to

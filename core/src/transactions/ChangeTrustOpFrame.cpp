...
ChangeTrustOpFrame::doApply(Application &app, LedgerDelta &delta,
    ...
    if (app.getLedgerManager().getCurrentLedgerVersion() > 2) {
        if (issuer && (issuer->getID() == getSourceID())) { // since version 3 it is
            // not allowed to use
            // CHANGE_TRUST on self
            app.getMetrics().newMeter({"op-change-trust", "failure", "trust-self"}, "operation").mark();
            innerResult().code(CHANGE_TRUST_SELF_NOT_ALLOWED);
            return false;
        }
    }

    if (trustLine) { // we are modifying an old trustline

        if (mChangeTrust.limit < trustLine->getMinimumLimit(ledgerManager)) { // Can't drop the limit
            // below the balance you
            // are holding with them
            app.getMetrics().newMeter({"op-change-trust", "failure", "invalid-limit"}, "operation").mark();
        ...
        if (mChangeTrust.limit == 0) {
            ...
            if (!issuer) {
                app.getMetrics().newMeter({"op-change-trust", "failure", "no-issuer"}, "operation").mark();
        ...
        app.getMetrics().newMeter({"op-change-trust", "success", "apply"}, "operation").mark();
        ...
        if (!mSourceAccount->addNumEntries(1, ledgerManager)) {
            app.getMetrics().newMeter({"op-change-trust", "failure", "low-reserve"}, "operation").mark();
...
ChangeTrustOpFrame::doCheckValid(Application &app) {
    if (mChangeTrust.limit < 0) {
        app.getMetrics().newMeter({"op-change-trust", "invalid", "malformed-negative-limit"}, "operation").mark();
        innerResult().code(CHANGE_TRUST_MALFORMED);
        return false;
    }
    if (!isAssetValid(mChangeTrust.line)) {
        app.getMetrics().newMeter({"op-change-trust", "invalid", "malformed-invalid-asset"}, "operation").mark();
        innerResult().code(CHANGE_TRUST_MALFORMED);
        return false;
    }

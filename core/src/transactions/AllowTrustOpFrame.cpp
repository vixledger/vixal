...
AllowTrustOpFrame::doApply(Application &app, LedgerDelta &delta,
    ...
    if (ledgerManager.getCurrentLedgerVersion() > 2) {
        if (mAllowTrust.trustor == getSourceID()) { // since version 3 it is not
            // allowed to use ALLOW_TRUST on
            // self
            app.getMetrics().newMeter({"op-allow-trust", "failure", "trust-self"}, "operation").mark();
            innerResult().code(ALLOW_TRUST_SELF_NOT_ALLOWED);
            return false;
        }
    }

    if (!(mSourceAccount->getAccount().flags & AUTH_REQUIRED_FLAG)) { // this account doesn't require authorization to
        // hold credit
        app.getMetrics().newMeter({"op-allow-trust", "failure", "not-required"}, "operation").mark();
    ...
    if (!(mSourceAccount->getAccount().flags & AUTH_REVOCABLE_FLAG) &&
        ...
        app.getMetrics().newMeter({"op-allow-trust", "failure", "cant-revoke"}, "operation").mark();
    ...
    if (!trustLine) {
        app.getMetrics().newMeter({"op-allow-trust", "failure", "no-trust-line"}, "operation").mark();
        innerResult().code(ALLOW_TRUST_NO_TRUST_LINE);
        return false;
    }

    app.getMetrics().newMeter({"op-allow-trust", "success", "apply"}, "operation").mark();
...
AllowTrustOpFrame::doCheckValid(Application &app) {
    if (mAllowTrust.asset.type() == ASSET_TYPE_NATIVE) {
        app.getMetrics().newMeter({"op-allow-trust", "invalid", "malformed-non-alphanum"}, "operation").mark();
    ...
    if (!isAssetValid(ci)) {
        app.getMetrics().newMeter({"op-allow-trust", "invalid", "malformed-invalid-asset"}, "operation").mark();
        innerResult().code(ALLOW_TRUST_MALFORMED);
        return false;
    }

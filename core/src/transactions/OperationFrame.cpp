...
OperationFrame::checkSignature(SignatureChecker &signatureChecker) const {
    auto neededThreshold = getNeededThreshold(*mSourceAccount, getThresholdLevel());
    return mParentTx.checkSignature(signatureChecker, *mSourceAccount, neededThreshold);
}
...
OperationFrame::checkValid(SignatureChecker &signatureChecker, Application &app,
    ...
    if (!loadAccount(app.getLedgerManager().getCurrentLedgerVersion(), delta,
        ...
        if (forApply || !mOperation.sourceAccount) {
            app.getMetrics().newMeter({"operation", "invalid", "no-account"}, "operation").mark();
            mResult.code(opNO_ACCOUNT);
            return false;
        } else {
            mSourceAccount = AccountFrame::makeAuthOnlyAccount(*mOperation.sourceAccount);
        }
    }

    if (!checkSignature(signatureChecker)) {
        app.getMetrics().newMeter({"operation", "invalid", "bad-auth"}, "operation").mark();

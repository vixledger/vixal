...
SetOptionsOpFrame::doApply(Application &app, LedgerDelta &delta,
    ...
    if (mSetOptions.inflationDest) {
        ...
        if (!inflationAccount) {
            app.getMetrics().newMeter({"op-set-options", "failure", "invalid-inflation"}, "operation").mark();
    ...
    if (mSetOptions.clearFlags) {
        if ((*mSetOptions.clearFlags & allAccountAuthFlags) &&
            ...
            app.getMetrics().newMeter({"op-set-options", "failure", "cant-change"}, "operation").mark();
    ...
    if (mSetOptions.masterWeight) {
        account.thresholds[THRESHOLD_MASTER_WEIGHT] = static_cast<unsigned char>(*mSetOptions.masterWeight & UINT8_MAX);
    }

    if (mSetOptions.lowThreshold) {
        account.thresholds[THRESHOLD_LOW] = static_cast<unsigned char>(*mSetOptions.lowThreshold & UINT8_MAX);
    }

    if (mSetOptions.medThreshold) {
        account.thresholds[THRESHOLD_MED] = static_cast<unsigned char>(*mSetOptions.medThreshold & UINT8_MAX);
    }

    if (mSetOptions.highThreshold) {
        account.thresholds[THRESHOLD_HIGH] = static_cast<unsigned char>(*mSetOptions.highThreshold & UINT8_MAX);
    }

    if (mSetOptions.signer) {
        auto &signers = account.signers;
        if (mSetOptions.signer->weight) { // add or change signer
            bool found = false;
            ...
            if (!found) {
                if (signers.size() == signers.max_size()) {
                    app.getMetrics().newMeter({"op-set-options", "failure", "too-many-signers"}, "operation").mark();
                    innerResult().code(SET_OPTIONS_TOO_MANY_SIGNERS);
                    return false;
                }
                if (!mSourceAccount->addNumEntries(1, ledgerManager)) {
                    app.getMetrics().newMeter({"op-set-options", "failure", "low-reserve"}, "operation").mark();
    ...
    app.getMetrics().newMeter({"op-set-options", "success", "apply"}, "operation").mark();
...
SetOptionsOpFrame::doCheckValid(Application &app) {
    ...
    if (mSetOptions.setFlags && mSetOptions.clearFlags) {
        if ((*mSetOptions.setFlags & *mSetOptions.clearFlags) != 0) {
            app.getMetrics().newMeter({"op-set-options", "invalid", "bad-flags"}, "operation").mark();
    ...
    if (mSetOptions.masterWeight) {
        if (*mSetOptions.masterWeight > UINT8_MAX) {
            app.getMetrics().newMeter({"op-set-options", "invalid", "threshold-out-of-range"}, "operation").mark();
    ...
    if (mSetOptions.signer) {
        ...
        if (isSelf || (!isPublicKey &&
            ...
            app.getMetrics().newMeter({"op-set-options", "invalid", "bad-signer"}, "operation").mark();
    ...
    if (mSetOptions.homeDomain) {
        if (!isString32Valid(*mSetOptions.homeDomain)) {
            app.getMetrics().newMeter({"op-set-options", "invalid", "invalid-home-domain"}, "operation").mark();

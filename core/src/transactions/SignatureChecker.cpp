...
SignatureChecker::SignatureChecker(
        ...
        : mProtocolVersion{protocolVersion}, mContentsHash{contentsHash}, mSignatures{signatures} {
...
SignatureChecker::checkSignature(AccountID const &accountID,
    ...
    auto verifyAll = [&](std::vector<Signer> &signers1, VerifyT verify) {
        for (size_t i = 0; i < mSignatures.size(); i++) {
            auto const &sig = mSignatures[i];

            for (auto it = signers1.begin(); it != signers1.end(); ++it) {
                auto &signerKey = *it;
    ...
    auto verified = verifyAll(signers[SIGNER_KEY_TYPE_HASH_X],
    ...
    verified = verifyAll(signers[SIGNER_KEY_TYPE_ED25519],
                         [&](DecoratedSignature const &sig, Signer const &signerKey) {

...
PathPaymentOpFrame::PathPaymentOpFrame(Operation const &op,
        ...
        : OperationFrame(op, res, parentTx), mPathPayment(mOperation.body.pathPaymentOp()) {
}
...
PathPaymentOpFrame::doApply(Application &app, LedgerDelta &delta,
    ...
    fullPath.insert(fullPath.end(), mPathPayment.path.begin(), mPathPayment.path.end());
    ...
    if (!bypassIssuerCheck) {
        ...
        if (!destination) {
            app.getMetrics().newMeter({"op-path-payment", "failure", "no-destination"}, "operation").mark();
    ...
    if (curB.type() == ASSET_TYPE_NATIVE) {
        ...
        if (bypassIssuerCheck) {
            destLine = TrustFrame::loadTrustLine(mPathPayment.destination, curB, db, &delta);
        } else {
            auto tlI = TrustFrame::loadTrustLineIssuer(mPathPayment.destination, curB, db, delta);
            if (!tlI.second) {
                app.getMetrics().newMeter({"op-path-payment", "failure", "no-issuer"}, "operation").mark();
        ...
        if (!destLine) {
            app.getMetrics().newMeter({"op-path-payment", "failure", "no-trust"}, "operation").mark();
            innerResult().code(PATH_PAYMENT_NO_TRUST);
            return false;
        }

        if (!destLine->isAuthorized()) {
            app.getMetrics().newMeter({"op-path-payment", "failure", "not-authorized"}, "operation").mark();
    ...
    for (int i = (int) fullPath.size() - 1; i >= 0; i--) {
        ...
        OfferExchange::ConvertResult r = oe.convertWithOffers(
                ...
                true, [this, &metrics](OfferFrame const &o) {
                    if (o.getSellerID() == getSourceID()) {
                        ...
                        metrics.newMeter({"op-path-payment", "failure", "offer-cross-self"}, "operation").mark();
        ...
        switch (r) {
            ...
            case OfferExchange::ePartial:
                app.getMetrics().newMeter({"op-path-payment", "failure", "too-few-offers"}, "operation").mark();
        ...
        offers.insert(offers.begin(), oe.getOfferTrail().begin(), oe.getOfferTrail().end());
    }
    ...
    if (curBSent > mPathPayment.sendMax) { // make sure not over the max
        app.getMetrics().newMeter({"op-path-payment", "failure", "over-send-max"}, "operation").mark();
    ...
    if (curB.type() == ASSET_TYPE_NATIVE) {
        ...
        if (ledgerManager.getCurrentLedgerVersion() > 7) {
            sourceAccount = AccountFrame::loadAccount(delta, mSourceAccount->getID(), db);

            if (!sourceAccount) {
                app.getMetrics().newMeter({"op-path-payment", "invalid", "no-account"}, "operation").mark();
                innerResult().code(PATH_PAYMENT_MALFORMED);
                return false;
            }
        }
        ...
        if (bypassIssuerCheck) {
            sourceLineFrame = TrustFrame::loadTrustLine(getSourceID(), curB, db, &delta);
        } else {
            auto tlI = TrustFrame::loadTrustLineIssuer(getSourceID(), curB, db, delta);
        ...
        if (!sourceLineFrame) {
            app.getMetrics().newMeter({"op-path-payment", "failure", "src-no-trust"}, "operation").mark();
    ...
    app.getMetrics().newMeter({"op-path-payment", "success", "apply"}, "operation").mark();
    return true;
}
...
PathPaymentOpFrame::doCheckValid(Application &app) {
    if (mPathPayment.destAmount <= 0 || mPathPayment.sendMax <= 0) {
        app.getMetrics().newMeter({"op-path-payment", "invalid", "malformed-amounts"}, "operation").mark();
        innerResult().code(PATH_PAYMENT_MALFORMED);
        return false;
    }

...
ManageDataOpFrame::ManageDataOpFrame(Operation const &op, OperationResult &res,
        ...
        : OperationFrame(op, res, parentTx), mManageData(mOperation.body.manageDataOp()) {
}
...
ManageDataOpFrame::doApply(Application &app, LedgerDelta &delta,
    ...
    if (app.getLedgerManager().getCurrentLedgerVersion() == 3) {
        throw std::runtime_error("MANAGE_DATA not supported on ledger version 3");
    }
    ...
    if (mManageData.dataValue) {
        if (!dataFrame) { // create a new data entry

            if (!mSourceAccount->addNumEntries(1, ledgerManager)) {
                app.getMetrics().newMeter({"op-manage-data", "invalid", "low reserve"}, "operation").mark();
    ...
    } else { // delete an existing piece of data

        if (!dataFrame) {
            app.getMetrics().newMeter({"op-manage-data", "invalid", "not-found"}, "operation").mark();
    ...
    app.getMetrics().newMeter({"op-manage-data", "success", "apply"}, "operation").mark();
    return true;
}
...
ManageDataOpFrame::doCheckValid(Application &app) {
    if (app.getLedgerManager().getCurrentLedgerVersion() < 2) {
        app.getMetrics().newMeter({"op-set-options", "invalid", "invalid-data-old-protocol"}, "operation").mark();
    ...
    if ((mManageData.dataName.size() < 1) ||
        ...
        app.getMetrics().newMeter({"op-set-options", "invalid", "invalid-data-name"}, "operation").mark();

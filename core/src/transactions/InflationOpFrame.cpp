...
constexpr const uint32_t INFLATION_FREQUENCY = (60 * 60 * 24 * 7); // every 7 days
// inflation is .000190721 per 7 days, or 1% a year
constexpr const int64_t INFLATION_RATE_TRILLIONTHS = 190721000LL;
constexpr const int64_t TRILLION = 1000000000000LL;
constexpr const int64_t INFLATION_WIN_MIN_PERCENT = 500000000LL; // .05%
constexpr const int INFLATION_NUM_WINNERS = 2000;
constexpr const time_t INFLATION_START_TIME = (1404172800LL); // 1-jul-2014 (unix epoch)
...
InflationOpFrame::doApply(Application &app, LedgerDelta &delta,
    ...
    auto closeTime = static_cast<time_t>(lcl.scpValue.closeTime);
    uint64_t seq = lcl.inflationSeq;

    auto inflationTime = static_cast<time_t>(INFLATION_START_TIME + seq * INFLATION_FREQUENCY);
    if (closeTime < inflationTime) {
        app.getMetrics().newMeter({"op-inflation", "failure", "not-time"}, "operation").mark();
    ...
    */
    ...
    int64_t minBalance = bigDivide(totalVotes, INFLATION_WIN_MIN_PERCENT, TRILLION, ROUND_DOWN);

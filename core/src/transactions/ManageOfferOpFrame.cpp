...
ManageOfferOpFrame::doApply(Application &app, LedgerDelta &delta,
    ...
    if (offerID) { // modifying an old offer
        mSellSheepOffer = OfferFrame::loadOffer(getSourceID(), offerID, db, &delta);
    ...
    if (mManageOffer.amount == 0) {
        ...
        if (wheat.type() == ASSET_TYPE_NATIVE) {
            ...
            if (maxWheatCanBuy == 0) {
                app.getMetrics().newMeter({"op-manage-offer", "invalid", "line-full"}, "operation").mark();
...

        {
            ...
            if (!bigDivide(maxSheepBasedOnWheat, maxWheatCanBuy, sheepPrice.d, sheepPrice.n, ROUND_DOWN)) {

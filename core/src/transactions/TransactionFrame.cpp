...
TransactionFrame::checkSignature(SignatureChecker &signatureChecker,
    ...
    if (account.getAccount().thresholds[0]) {
        signers.emplace_back(KeyUtils::convertKey<SignerKey>(account.getID()),
...
TransactionFrame::commonValidPreSeqNum(Application &app, LedgerDelta *delta) {
    ...
    if (mOperations.size() == 0) {
        app.getMetrics().newMeter({"transaction", "invalid", "missing-operation"}, "transaction").mark();
    ...
    if (mEnvelope.tx.timeBounds) {
        ...
        if (mEnvelope.tx.timeBounds->minTime > closeTime) {
            app.getMetrics().newMeter({"transaction", "invalid", "too-early"}, "transaction").mark();
            getResult().result.code(txTOO_EARLY);
            return false;
        }
        if (mEnvelope.tx.timeBounds->maxTime &&
            (mEnvelope.tx.timeBounds->maxTime < closeTime)) {
            app.getMetrics().newMeter({"transaction", "invalid", "too-late"}, "transaction").mark();
    ...
    if (mEnvelope.tx.fee < getMinFee(lm)) {
        app.getMetrics().newMeter({"transaction", "invalid", "insufficient-fee"}, "transaction").mark();
    ...
    if (!loadAccount(app.getLedgerManager().getCurrentLedgerVersion(), delta,
                     app.getDatabase())) {
        app.getMetrics().newMeter({"transaction", "invalid", "no-account"}, "transaction").mark();
...
TransactionFrame::commonValid(SignatureChecker &signatureChecker,
    ...
    if (lm.getCurrentLedgerVersion() >= 10 || !applying) {
        ...
        if (current == INT64_MAX || current + 1 != mEnvelope.tx.seqNum) {
            ...
            app.getMetrics().newMeter({"transaction", "invalid", "bad-seq"}, "transaction").mark();
...
TransactionFrame::checkValid(Application &app, SequenceNumber current) {
    ...
    bool res = commonValid(signatureChecker, app, nullptr, current) == ValidationType::kFullyValid;
    if (res) {
        for (auto &op : mOperations) {
            if (!op->checkValid(signatureChecker, app)) {
                ...
                app.getMetrics().newMeter({"transaction", "invalid", "invalid-op"}, "transaction").mark();
                markResultFailed();
                return false;
            }
        }
...
VixalMessage
TransactionFrame::toVixalMessage() const {
    VixalMessage msg;
...
TransactionFrame::copyTransactionsToStream(Hash const &networkID, Database &db,
    ...
    while (st.got_data()) {
        if (curLedgerSeq != lastLedgerSeq) {
            saveTransactionHelper(db, sess, lastLedgerSeq, txSet, results, txOut, txResultOut);
            // reset state
        ...
        TransactionFramePtr txFrame = make_shared<TransactionFrame>(networkID, tx);
        txSet.add(txFrame);

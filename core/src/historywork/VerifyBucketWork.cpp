...
VerifyBucketWork::VerifyBucketWork(
        ...
        : Work(app, parent, std::string("verify-bucket-hash ") + bucketFile,
               RETRY_NEVER), mBuckets(buckets), mBucketFile(bucketFile), mHash(hash),
          mVerifyBucketSuccess(app.getMetrics().newMeter({"history", "verify-bucket", "success"}, "event")),
          mVerifyBucketFailure(app.getMetrics().newMeter({"history", "verify-bucket", "failure"}, "event")) {
...
VerifyBucketWork::onStart() {
    ...
    asio::post(app.io_context(), [&app, filename, handler, hash]() {
        ...
        char buf[4096];
        {
            ...
            while (in) {
                ...
                hasher->add(ByteSlice(buf, static_cast<size_t>(in.gcount())));
            }
            uint256 vHash = hasher->finish();
            if (vHash == hash) {
                CLOG(DEBUG, "History") << "Verified hash (" << hexAbbrev(hash) << ") for " << filename;
            } else {
        ...
        asio::post(app.getClock().io_context(), [ec, handler]() { handler(ec); });
    });
}
...
VerifyBucketWork::onSuccess() {
    auto b = mApp.getBucketManager().adoptFileAsBucket(mBucketFile, mHash, 0, 0);

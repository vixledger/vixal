...
PublishWork::PublishWork(Application &app, AbstractWork &parent,
                         std::shared_ptr<StateSnapshot> snapshot)
        : Work(app, parent,
               fmt::format("publish-{:08x}", snapshot->mLocalState.currentLedger)), mSnapshot(snapshot),
...
PublishWork::onSuccess() {
    ...
    if (!mUpdateArchivesWork) {
        ...
        for (auto &writableArchive : mApp.getHistoryArchiveManager().getWritableHistoryArchives()) {

...
ResolveSnapshotWork::ResolveSnapshotWork(
        ...
        : Work(app, parent, "prepare-snapshot", Work::RETRY_FOREVER), mSnapshot(snapshot) {
}
...
ResolveSnapshotWork::onRun() {
    ...
    mSnapshot->makeLive();
    if ((mApp.getLedgerManager().getLastClosedLedgerNum() >  mSnapshot->mLocalState.currentLedger) &&

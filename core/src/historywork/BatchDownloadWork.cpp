...
BatchDownloadWork::BatchDownloadWork(Application &app, AbstractWork &parent,
                                     CheckpointRange range,
                                     std::string const &type,
                                     TmpDir const &downloadDir)
        : Work(app, parent, fmt::format("batch-download-{:s}-{:08x}-{:08x}", type,
                                        range.first(), range.last())), mRange(range), mNext(mRange.first()),
          mFileType(type), mDownloadDir(downloadDir), mDownloadCached(app.getMetrics().newMeter(
                {"history", "download-" + type, "cached"}, "event")), mDownloadStart(app.getMetrics().newMeter(
                {"history", "download-" + type, "start"}, "event")), mDownloadSuccess(app.getMetrics().newMeter(
                {"history", "download-" + type, "success"}, "event")), mDownloadFailure(app.getMetrics().newMeter(

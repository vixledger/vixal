...
HistoryManagerImpl::HistoryManagerImpl(Application &app)
        : mApp(app), mWorkDir(nullptr), mPublishWork(nullptr), mPublishSkip(
        app.getMetrics().newMeter({"history", "publish", "skip"}, "event")), mPublishQueue(
        app.getMetrics().newMeter({"history", "publish", "queue"}, "event")), mPublishDelay(
        app.getMetrics().newMeter({"history", "publish", "delay"}, "event")), mPublishStart(
        app.getMetrics().newMeter({"history", "publish", "start"}, "event")), mPublishSuccess(
        app.getMetrics().newMeter({"history", "publish", "success"}, "event")), mPublishFailure(
...
HistoryManagerImpl::selectRandomReadableHistoryArchive() {
    ...
    if (archives.size() == 0) {
        ...
        CLOG(DEBUG, "History") << "Fetching from sole readable history archive '"
                               << archives[0].first << "'";
...
HistoryManagerImpl::getMinLedgerQueuedToPublish() {
    ...
    auto prep = mApp.getDatabase().getPreparedStatement("SELECT min(ledger) FROM publishqueue;");
    auto &st = prep.statement();
...
HistoryManagerImpl::getMaxLedgerQueuedToPublish() {
    ...
    auto prep = mApp.getDatabase().getPreparedStatement("SELECT max(ledger) FROM publishqueue;");
    auto &st = prep.statement();
...
HistoryManagerImpl::maybeQueueHistoryCheckpoint() {
    ...
    if (!hasAnyWritableHistoryArchive()) {
        mPublishSkip.mark();
        CLOG(DEBUG, "History") << "Skipping checkpoint, no writable history archives";
        return false;
    }
...
HistoryManagerImpl::historyPublished(
    ...
    mPublishWork.reset();
    asio::post(mApp.getClock().io_context(), [this]() { this->publishQueuedHistory(); });
}
...
HistoryManagerImpl::downloadMissingBuckets(
    ...
    mApp.getWorkManager().addWork<RepairMissingBucketsWork>(desiredState, handler);
    mApp.getWorkManager().advanceChildren();
}

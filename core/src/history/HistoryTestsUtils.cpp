...
CatchupMetrics::CatchupMetrics()
        : mHistoryArchiveStatesDownloaded{0}, mLedgersDownloaded{0}, mLedgersVerified{0},
          mLedgerChainsVerificationFailed{0}, mBucketsDownloaded{false}, mBucketsApplied{false},
          mTransactionsDownloaded{0}, mTransactionsApplied{0} {
}
...
CatchupMetrics::CatchupMetrics(
        ...
        : mHistoryArchiveStatesDownloaded{historyArchiveStatesDownloaded}, mLedgersDownloaded{ledgersDownloaded},
          mLedgersVerified{ledgersVerified}, mLedgerChainsVerificationFailed{ledgerChainsVerificationFailed},
          mBucketsDownloaded{bucketsDownloaded}, mBucketsApplied{bucketsApplied},
          mTransactionsDownloaded{transactionsDownloaded}, mTransactionsApplied{transactionsApplied} {
}
...
CatchupPerformedWork::CatchupPerformedWork(CatchupMetrics const &metrics)
        : mHistoryArchiveStatesDownloaded{metrics.mHistoryArchiveStatesDownloaded},
          mLedgersDownloaded{metrics.mLedgersDownloaded}, mLedgersVerified{metrics.mLedgersVerified},
          mLedgerChainsVerificationFailed{metrics.mLedgerChainsVerificationFailed},
          mBucketsDownloaded{metrics.mBucketsDownloaded > 0}, mBucketsApplied{metrics.mBucketsApplied > 0},
          mTransactionsDownloaded{metrics.mTransactionsDownloaded}, mTransactionsApplied{metrics.mTransactionsApplied} {
}
...
CatchupSimulation::generateRandomLedger() {
    ...
    VixalValue sv(txSet->getContentsHash(), closeTime, emptyUpgradeSteps, 0);
    mLedgerCloseData.emplace_back(ledgerSeq, txSet, sv);
    lm.closeLedger(mLedgerCloseData.back());
    ...
    if (!manual) {
        ...
        for (uint32_t n = initLedger + 1; n <= triggerLedger; ++n) {
            ...
            if (n == gap) {
                ...
                // Remember the vectors count from 2, not 0.
                auto const &lcd = mLedgerCloseData.at(n - 2);
                CLOG(INFO, "History")

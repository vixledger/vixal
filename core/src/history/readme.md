...
The `get` and `put` commands define subprocesses that `vixal-core` will
...
called `s3vixal` based on the [AWS S3](https://aws.amazon.com/s3/) storage system:

~~~~
[HISTORY.s3vixal]
get="curl http://history.vixal.org/{0} -o {1}"
put="aws s3 cp {0} s3://history.vixal.org/{1}"
~~~~

In this example, `vixal-core` will use the `curl` command to fetch history and

...
ItemFetcher::ItemFetcher(Application &app, AskPeer askPeer)
        : mApp(app),
          mItemMapSize(app.getMetrics().newCounter({"overlay", "memory", "item-fetch-map"})), mAskPeer(askPeer) {
}
...
ItemFetcher::fetch(Hash itemHash, const SCPEnvelope &envelope) {
    ...
    if (entryIt == mTrackers.end()) { // not being tracked
        TrackerPtr tracker =
...
ItemFetcher::stopFetchingBelow(uint64 slotIndex) {
    ...
    asio::post(mApp.getClock().io_context(), [this, slotIndex]() { stopFetchingBelowInternal(slotIndex); });
}
...
ItemFetcher::recv(Hash itemHash) {
    ...
    if (iter != mTrackers.end()) {
        // this code can safely be called even if recvSCPEnvelope ends up calling recv on the same itemHash

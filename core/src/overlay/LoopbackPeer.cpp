...
LoopbackPeer::drop(bool) {
    ...
    if (remote) {
        asio::post(remote->getApp().getClock().io_context(), [remote]() { remote->drop(); });
    }
}
...
LoopbackPeer::processInQueue() {
    if (!mInQueue.empty() && mState != CLOSING) {
        ...
        if (!mInQueue.empty()) {
            ...
            asio::post(mApp.getClock().io_context(), [self]() { self->processInQueue(); });
        }
    }
}
...
LoopbackPeer::deliverOne() {
    ...
    if (!mOutQueue.empty() && !mCorked) {
        ...
        if (remote) {
            ...
            asio::post(remote->getApp().getClock().io_context(), [remote]() { remote->processInQueue(); });
        }
...
LoopbackPeerConnection::LoopbackPeerConnection(Application &initiator,
    ...
    // connectHandler
    if (mInitiator->mState != Peer::CONNECTED || mAcceptor->mState != Peer::CONNECTED) {
        return;
    }
    ...
    asio::post(mInitiator->getApp().getClock().io_context(),

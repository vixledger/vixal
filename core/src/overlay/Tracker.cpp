...
Tracker::Tracker(Application &app, Hash const &hash, AskPeer &askPeer)
        : mAskPeer(askPeer), mApp(app), mNumListRebuild(0),
          mTimer(app.getClock()), mItemHash(hash),
          mTryNextPeerReset(app.getMetrics().newMeter({"overlay", "item-fetcher", "reset-fetcher"}, "item-fetcher")),
          mTryNextPeer(app.getMetrics().newMeter({"overlay", "item-fetcher", "next-peer"}, "item-fetcher")) {
    assert(mAskPeer);
}
...
Tracker::tryNextPeer() {
    ...
    if (mPeersToAsk.empty() && !mLastAskedPeer) {
        ...
        for (auto const &p :mApp.getOverlayManager().getRandomAuthenticatedPeers()) {
    ...
    if (!peer) { // we have asked all our peers
        ...
        mLastAskedPeer = peer;
        CLOG(TRACE, "Overlay") << "Asking for " << hexAbbrev(mItemHash) << " to " << peer->toString();
        mTryNextPeer.mark();
        mAskPeer(peer, mItemHash);
        nextTry = MS_TO_WAIT_FOR_FETCH_REPLY;
    }

    mTimer.expires_after(nextTry);

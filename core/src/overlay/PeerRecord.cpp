...
PeerRecord::loadPeerRecord(Database &db, string ip, unsigned short port) {
    ...
    if (st.got_data()) {
        return make_optional<PeerRecord>(ip, port, VirtualClock::tmToPoint(tm), numFailures);
    } else {
        return nullopt;
    }
}
        ...
        do {
            ...
            auto prep = db.getPreparedStatement(
                    "SELECT ip, port, nextattempt, numfailures "
                            ...
                            "ORDER BY nextattempt ASC, numfailures ASC limit :max OFFSET :o");
            auto &st = prep.statement();
...
PeerRecord::computeBackoff(VirtualClock &clock) {
    ...
    auto nsecs = std::chrono::seconds(
            random_t::get<uint32>(0, (static_cast<uint32>(std::pow(2, backoffCount) * SECONDS_PER_BACKOFF))));

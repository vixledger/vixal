...
static const char *loadPeerRecordSelector = "SELECT ip, port, nextattempt, numfailures, flags FROM peers ";
...
PeerRecord::computeBackoff(VirtualClock &clock) {
    ...
    auto nsecs = std::chrono::seconds(
            random_t::get<uint32>(0, (static_cast<uint32>(std::pow(2, backoffCount) * SECONDS_PER_BACKOFF) + 1)));

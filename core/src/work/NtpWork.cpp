...
NtpWork::NtpWork(Application &app, AbstractWork &parent, std::string ntpServer,
                 std::chrono::seconds tolerance)
        : Work(app, parent, "ntp-work", RETRY_FOREVER),
          mNtpServer(std::move(ntpServer)), mTolerance(tolerance),
          mTimeoutTimer(app.getClock()), mIsSynchronized(true) {
...
NtpWork::onRun() {
    ...
    mNtpClient = std::make_shared<NtpClient>(app().io_context(), mNtpServer,
                                             [weak](long time) {
                                                 auto self = weak.lock();
                                                 if (self) {
    ...
    mTimeoutTimer.expires_after(std::chrono::seconds(5));

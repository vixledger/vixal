...
Database::Database(Application &app)
        : mApp(app), mQueryMeter(
        app.getMetrics().newMeter({"database", "query", "exec"}, "query")), mStatementsSize(
        app.getMetrics().newCounter({"database", "memory", "statements"})), mEntryCache(4096), mExcludedQueryTime(0),
          mExcludedTotalTime(0), mLastIdleQueryTime(0), mLastIdleTotalTime(app.getClock().now()) {
    registerDrivers();
...
Database::putSchemaVersion(unsigned long vers) {
    mApp.getPersistentState().setState(PersistentState::kDatabaseSchema, std::to_string(vers));
}

unsigned long
Database::getDBSchemaVersion() {
    auto vstr = mApp.getPersistentState().getState(PersistentState::kDatabaseSchema);
...
Database::isSqlite() const {
    return mApp.getConfig().DATABASE.value.find("sqlite3:") != std::string::npos;
}
...
public:
    SQLLogContext(std::string name, soci::session &sess)
            : mName(std::move(name)), mSess(sess) {
...
DBTimeExcluder::DBTimeExcluder(Application &app)
        : mApp(app), mStartQueryTime(app.getDatabase().totalQueryTime()), mStartTotalTime(app.getClock().now()) {
}

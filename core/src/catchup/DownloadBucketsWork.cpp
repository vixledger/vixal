...
DownloadBucketsWork::DownloadBucketsWork(
        ...
        : Work{app, parent, "download-and-verify-buckets"},
          mBuckets{buckets}, mHashes{std::move(hashes)},
          mDownloadDir{downloadDir},
          mDownloadBucketStart{app.getMetrics().newMeter({"history", "download-bucket", "start"}, "event")},
          mDownloadBucketSuccess{app.getMetrics().newMeter({"history", "download-bucket", "success"}, "event")},
          mDownloadBucketFailure{app.getMetrics().newMeter({"history", "download-bucket", "failure"}, "event")} {
}

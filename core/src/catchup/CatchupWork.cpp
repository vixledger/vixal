...
CatchupWork::CatchupWork(Application &app, AbstractWork &parent,
                         CatchupConfiguration catchupConfiguration,
                         bool manualCatchup, ProgressHandler progressHandler,
                         size_t maxRetries)
        : BucketDownloadWork(app, parent, "catchup",
                             app.getHistoryManager().getLastClosedHistoryArchiveState(), maxRetries),
...
CatchupWork::applyBuckets() {
    ...
    assert(mApplyBucketsRemoteState.currentLedger == mFirstVerified.header.ledgerSeq);
    assert(mApplyBucketsRemoteState.getBucketListHash() == mFirstVerified.header.bucketListHash);
    ...
    if (mFirstVerified.header.ledgerSeq < lcl.header.ledgerSeq) {
        throw std::runtime_error(fmt::format("Catchup MINIMAL applying ledger earlier than local LCL: {:s} < {:s}",
                                             LedgerManager::ledgerAbbrev(mFirstVerified),
                                             LedgerManager::ledgerAbbrev(lcl)));
    }

    CLOG(INFO, "History") << "Catchup applying buckets for state " << LedgerManager::ledgerAbbrev(mFirstVerified);
    mApplyBucketsWork = addWork<ApplyBucketsWork>(mBuckets, mApplyBucketsRemoteState);
...
CatchupWork::onSuccess() {
    ...
    auto resolvedConfiguration = mCatchupConfiguration.resolve(mRemoteState.currentLedger);
    auto catchupRange = makeCatchupRange(mLastClosedLedgerAtReset, resolvedConfiguration, mApp.getHistoryManager());
    auto ledgerRange = catchupRange.first;
    auto checkpointRange = CheckpointRange{ledgerRange, mApp.getHistoryManager()};
    ...
    if (catchupRange.second) {
        ...
        CLOG(INFO, "History") << "Catchup downloading history archive state for applying buckets at checkpoint "
...
//
// if it is impossible, returns smallest possible ledger number - LedgerManager::GENESIS_LEDGER_SEQ
uint32_t
...
computeCatchupStart(uint32_t smallestLedgerToApply,
    ...
    auto smallestCheckpointToApply = historyManager.checkpointContainingLedger(smallestLedgerToApply);

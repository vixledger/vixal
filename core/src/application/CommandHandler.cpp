...
CommandHandler::CommandHandler(Application &app) : mApp(app) {
    if (mApp.getConfig().HTTP_PORT) {
        ...
        mServer = std::make_unique<http::server::server>(
                app.getClock().io_context(), ipStr, mApp.getConfig().HTTP_PORT,
                httpMaxClient);
    } else {
        mServer = std::make_unique<http::server::server>(
                app.getClock().io_context());
    }
...
CommandHandler::fileNotFound(std::string const &params, std::string &retStr) {
    retStr = "<b>Welcome to vixal-core!</b><p>";
    ...
    retStr += "<p><h1> /bans</h1>"
              ...
              "</p><p><h1> "
              "/generateload[?mode=(create|pay)&accounts=N&offset=K&txs=M&txrate=(R|auto)&batchsize=L]</h1>"
...
CommandHandler::manualClose(std::string const &params, std::string &retStr) {
    if (mApp.manualClose()) {
        ...
        retStr = "Set MANUAL_CLOSE=true in the vixal-core.cfg if you want this behavior";
    }
}
...
CommandHandler::upgrades(std::string const &params, std::string &retStr) {
    ...
    if (s == "get") {
        ...
        try {
            ...
            retStr = fmt::format("could not parse upgradetime: '{}'", upgradeTime);
            return;
        }
        ...
        p.mProtocolVersion = maybeParseParam(retMap, "protocolversion", protocolVersion);
...
CommandHandler::setcursor(std::string const &params, std::string &retStr) {
    ...
    auto cursor = parseParam<uint32>(map, "cursor");

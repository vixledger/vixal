    ...
    try {
        ...
        for (auto &item : g) {
            ...
            if (item.first == "PEER_PORT") {
                ...
                ARTIFICIALLY_SET_CLOSE_TIME_FOR_TESTING = readInt<uint32_t>(item, 0, UINT32_MAX - 1);
                ...
                AUTOMATIC_MAINTENANCE_PERIOD = std::chrono::seconds{readInt<uint32_t>(item)};
                ...
                MAX_ADDITIONAL_PEER_CONNECTIONS = readInt<int>(item, -1, UINT16_MAX);
            } else if (item.first == "MAX_PENDING_CONNECTIONS") {
                MAX_PENDING_CONNECTIONS = readInt<unsigned short>(item, 1, UINT16_MAX);
            } else if (item.first == "PEER_AUTHENTICATION_TIMEOUT") {
                PEER_AUTHENTICATION_TIMEOUT = readInt<unsigned short>(item, 1, UINT16_MAX);
        ...
        if (g.contains("PREFERRED_PEER_KEYS")) {
            ...
            if (pkeys) {
                auto values = readStringArray(ConfigItem{"PREFERRED_PEER_KEYS", pkeys});
                for (auto v : values) {
                    PublicKey nodeID;
                    parseNodeID(v, nodeID);
        ...
        if (MAX_ADDITIONAL_PEER_CONNECTIONS > (UINT16_MAX - TARGET_PEER_CONNECTIONS)) {
            throw std::invalid_argument("invalid MAX_ADDITIONAL_PEER_CONNECTIONS");
        }

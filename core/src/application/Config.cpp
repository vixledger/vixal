...
Config::Config() : NODE_SEED(SecretKey::random()) {
    ...
    LOG_FILE_PATH = "vixal-core.%datetime{%Y.%M.%d-%H:%m:%s}.log";
...
Config::loadQset(std::shared_ptr<cpptoml::toml_group> group, SCPQuorumSet &qset,
    ...
    qset.threshold = uint32(1 + (((qset.validators.size() + qset.innerSets.size()) * thresholdPercent - 1) / 100));
    ...
    try {
        ...
        for (auto &item : g) {
            ...
            if (item.first == "PEER_PORT") {
                ...
                int64_t parsedTargetPeerConnections = item.second->as<int64_t>()->value();
                if (parsedTargetPeerConnections <= 0 || parsedTargetPeerConnections > UINT16_MAX) {
                    throw std::invalid_argument("invalid TARGET_PEER_CONNECTIONS");
                }
                TARGET_PEER_CONNECTIONS = static_cast<unsigned short>(parsedTargetPeerConnections);
            } else if (item.first == "MAX_PEER_CONNECTIONS") {
                if (!item.second->as<int64_t>()) {
                    throw std::invalid_argument("invalid MAX_PEER_CONNECTIONS");
                }
                int64_t parsedMaxPeerConnections = item.second->as<int64_t>()->value();
                if (parsedMaxPeerConnections <= 0 || parsedMaxPeerConnections > UINT16_MAX) {
                    throw std::invalid_argument("invalid MAX_PEER_CONNECTIONS");
                }
                MAX_PEER_CONNECTIONS = static_cast<unsigned short>(parsedMaxPeerConnections);
            } else if (item.first == "MAX_PENDING_CONNECTIONS") {
                if (!item.second->as<int64_t>()) {
                    throw std::invalid_argument("invalid MAX_PENDING_CONNECTIONS");
                }
                int64_t parsedMaxPendingConnections = item.second->as<int64_t>()->value();
                if (parsedMaxPendingConnections <= 0 || parsedMaxPendingConnections > UINT16_MAX) {
                    throw std::invalid_argument("invalid MAX_PENDING_CONNECTIONS");
                }
                MAX_PENDING_CONNECTIONS = static_cast<unsigned short>(parsedMaxPendingConnections);
            } else if (item.first == "PEER_AUTHENTICATION_TIMEOUT") {
                if (!item.second->as<int64_t>()) {
                    throw std::invalid_argument("invalid PEER_AUTHENTICATION_TIMEOUT");
                }
                auto v = item.second->as<int64_t>()->value();
                if (v <= 0 || v > UINT16_MAX) {
                    throw std::invalid_argument("bad PEER_AUTHENTICATION_TIMEOUT");
                }
                ...
                for (auto v : item.second->as_array()->array()) {
                    ...
                    PREFERRED_PEERS.emplace_back(v->as<std::string>()->value());
                }
                ...
                for (auto v : item.second->as_array()->array()) {
                    ...
                    KNOWN_PEERS.emplace_back(v->as<std::string>()->value());
                }
                ...
                for (auto v : item.second->as_array()->array()) {
                    ...
                    COMMANDS.emplace_back(v->as<std::string>()->value());
                }
                ...
                for (auto v : item.second->as_array()->array()) {
                    ...
                    INVARIANT_CHECKS.emplace_back(v->as<std::string>()->value());
                }
            } else {
...
Config::validateConfig() {
    ...
    LocalNode::forAllNodes(QUORUM_SET, [&](NodeID const &n) { nodes.insert(n); });
    ...
    if (FAILURE_SAFETY == -1) {
        // calculates default value for safety giving the top level entities the same weight
        // n = 3f+1 <=> f = (n-1)/3
...
Config::parseNodeID(std::string configStr, PublicKey &retKey, SecretKey &sKey,
    ...
    if (configStr[0] == '$') {
        ...
        if (iss) { // get any common name they have added
            ...
            if (commonName.size()) {
                ...
                if (!VALIDATOR_NAMES.emplace(std::make_pair(nodestr, commonName)).second) {
                    std::stringstream msg;

...
string const ExternalQueue::kSQLCreateStatement{
...
ExternalQueue::deleteCursor(std::string const &resid) {
    ...
    auto timer = mApp.getDatabase().getInsertTimer("pubsub");
    auto prep = mApp.getDatabase().getPreparedStatement("DELETE FROM pubsub WHERE resid = :n;");
    auto &st = prep.statement();
    st.exchange(soci::use(resid));
    st.define_and_bind();
    st.execute(true);
...
ExternalQueue::deleteOldEntries(uint32 count) {
    ...
    soci::statement st = (db.getSession().prepare << "SELECT MIN(lastread) FROM pubsub", soci::into(m, minIndicator));
    {
...
ExternalQueue::getCursor(std::string const &resid) {
    ...
    auto &db = mApp.getDatabase();
    auto prep = db.getPreparedStatement("SELECT lastread FROM pubsub WHERE resid = :n;");
    auto &st = prep.statement();

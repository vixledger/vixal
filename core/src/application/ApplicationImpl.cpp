...
ApplicationImpl::ApplicationImpl(VirtualClock &clock, Config const &cfg)
        : mVirtualClock(clock),
          ...
          mWorkerThreads(),
          mStopSignals(clock.io_context(), SIGINT),
          mStopping(false),
          mStoppingTimer(mVirtualClock),
    ...
    LOG(DEBUG) << "Application constructing " << "(worker threads: " << t << ")";
...
ApplicationImpl::initialize() {
    ...
    mTmpDirManager = std::make_unique<TmpDirManager>(mConfig.BUCKET_DIR_PATH + "/tmp");
    ...
    if (!mConfig.NTP_SERVER.empty()) {
        mNtpSynchronizationChecker = std::make_shared<NtpSynchronizationChecker>(*this, mConfig.NTP_SERVER);
    }
...
ApplicationImpl::timeNow() {
    return static_cast<uint64_t>(VirtualClock::to_time_t(getClock().now()));
}
...
ApplicationImpl::start() {
    ...
    if (!isQuorumSetSane(mConfig.QUORUM_SET, !mConfig.UNSAFE_QUORUM)) {
        std::string err("Invalid QUORUM_SET: duplicate entry or bad threshold (should be between ");
    ...
    mLedgerManager->loadLastKnownLedger(
            [this, &done](asio::error_code const &ec) {
                ...
                if (npub != 0) {
                    CLOG(INFO, "Ledger") << "Restarted publishing " << npub << " queued snapshots";
                }
...
ApplicationImpl::checkDB() {
    asio::post(getClock().io_context(), [this] {
...
ApplicationImpl::syncOwnMetrics() {
    ...
    mMetrics->newMeter({"crypto", "verify", "total"}, "signature").mark(vhit + vmiss);

    // Similarly, flush global process-table stats.
    mMetrics->newCounter({"process", "memory", "handles"}).set_count(mProcessManager->getNumRunningProcesses());
}
...
asio::io_context &
ApplicationImpl::io_context() {
    return io_context_;
}

void
ApplicationImpl::enableInvariantsFromConfig() {
    for (const auto &name : mConfig.INVARIANT_CHECKS) {

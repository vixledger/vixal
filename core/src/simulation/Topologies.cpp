...
Topologies::separate(int nNodes, double quorumThresholdFraction,
    ...
    for (int i = 0; i < nNodes; i++) {
        keys.push_back(SecretKey::fromSeed(sha256("NODE_SEED_" + to_string(i))));
    }

    SCPQuorumSet qSet;
    assert(quorumThresholdFraction >= 0.5);
    qSet.threshold = (uint32) min(nNodes, static_cast<int>(ceil(nNodes * quorumThresholdFraction)));
...
Topologies::hierarchicalQuorumSimplified(int coreSize, int nbOuterNodes,
    ...
    qSetBuilder.threshold = n - (n - 1) / 3u;
    vector<NodeID> coreNodeIDs;
    for (auto const &coreNodeID : sim->getNodeIDs()) {

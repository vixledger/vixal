...
Simulation::~Simulation() {
    ...
    mClock.io_context().poll_one();
    mClock.io_context().stop();
...
Simulation::addPendingConnection(NodeID const &initiator,
    ...
    mPendingConnections.emplace_back(initiator, acceptor);
}
...
Simulation::dropLoopbackConnection(NodeID initiator, NodeID acceptor) {
    auto it = std::find_if(
            ...
            [&](std::shared_ptr<LoopbackPeerConnection> const &conn) {
                return conn->getInitiator()->getApp().getConfig().NODE_SEED.getPublicKey() == initiator &&
                       conn->getAcceptor()->getApp().getConfig().NODE_SEED.getPublicKey() == acceptor;
            });
...
Simulation::crankAllNodes(int nbTicks) {
    ...
    do {
        ...
        if (mVirtualClockMode) {
            ...
            mainQuantumTimer.expires_after(quantum);
...
Simulation::haveAllExternalized(uint32 num, uint32 maxSpread) {
    ...
    if (max - min > maxSpread) {
        throw std::runtime_error(fmt::format("Too wide spread between nodes: {0}-{1} > {2}", max, min, maxSpread));
    }
    return num <= min;
}
...
Simulation::crankUntil(function<bool()> const &predicate, VirtualClock::duration timeout, bool finalCrank) {
    ...
    function<void()> checkDone = [&]() {
        if (predicate()) {
            ...
            checkTimer.expires_after(chrono::seconds(1));
    ...
    // initial check, pre crank mostly used for getting a snapshot of the starting state and being offset by
    // a to avoid being too synchronized with apps
    checkTimer.expires_after(chrono::microseconds(100));
...
Simulation::executeStressTest(size_t nTransactions, int injectionRatePerSec,
                              function<TxInfo(size_t i)> generatorFn) {
...
Simulation::accountsOutOfSyncWithDb() {
    ...
    for (auto const &p : mNodes) {
        ...
        for (auto accountIt = mAccounts.begin() + 1; accountIt != mAccounts.end(); accountIt++) {
            ...
            accountFrame = AccountFrame::loadAccount(account->mKey.getPublicKey(), app->getDatabase());
            int64_t offset;
            if (accountFrame) {

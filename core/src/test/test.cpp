...
getTestConfig(unsigned instanceNumber, Config::TestDbMode mode) {
    ...
    if (!cfgs[instanceNumber]) {
        gTestRoots.emplace_back("vixal-core-test");
        ...
        sstream << "vixal" << instanceNumber << ".log";
...
void
for_versions_from(uint32 from, Application &app, std::function<void(void)> const &f) {
...
void
for_versions(uint32 from, uint32 to, Application &app, std::function<void(void)> const &f) {
    if (from > to) {
        return;
    }
    auto versions = std::vector<uint32>{};
    versions.resize(to - from + 1u);

...
binToHex(ByteSlice const &bin) {
    ...
    if (sodium_bin2hex(hex.data(), hex.size(), bin.data(), bin.size()) != hex.data()) {
        throw std::runtime_error(
                "error in vixal::binToHex(std::vector<uint8_t>)");
    }
...
hexToBin(std::string const &hex) {
    ...
    if (sodium_hex2bin(bin.data(), bin.size(), hex.data(), hex.size(), NULL, NULL, NULL) != 0) {
        throw std::runtime_error("error in vixal::hexToBin(std::string)");
    }
    return bin;
}

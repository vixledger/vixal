...
namespace std {
size_t
hash<vixal::PublicKey>::operator()(vixal::PublicKey const &k) const noexcept {
    assert(k.type() == vixal::PUBLIC_KEY_TYPE_ED25519);

    return std::hash<vixal::uint256>()(k.ed25519());
}
}

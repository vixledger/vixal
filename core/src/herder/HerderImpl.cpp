...
HerderImpl::SCPMetrics::SCPMetrics(Application &app)
        : mLostSync(app.getMetrics().newMeter({"scp", "sync", "lost"}, "sync")),
          ...
          mKnownSlotsSize(app.getMetrics().newCounter({"scp", "memory", "known-slots"})),
          mCumulativeStatements(app.getMetrics().newCounter({"scp", "memory", "cumulative-statements"})),
          mHerderPendingTxs0(app.getMetrics().newCounter({"herder", "pending-txs", "age0"})),
          mHerderPendingTxs1(app.getMetrics().newCounter({"herder", "pending-txs", "age1"})),
          mHerderPendingTxs2(app.getMetrics().newCounter({"herder", "pending-txs", "age2"})),
          mHerderPendingTxs3(app.getMetrics().newCounter({"herder", "pending-txs", "age3"})) {
}
...
HerderImpl::HerderImpl(Application &app)
    ...
    mPendingEnvelopes.addSCPQuorumSet(hash, 0, getSCP().getLocalNode()->getQuorumSet());
}
...
HerderImpl::startRebroadcastTimer() {
    mRebroadcastTimer.expires_after(std::chrono::seconds(2));
...
HerderImpl::triggerNextLedger(uint32_t ledgerSeqToTrigger) {
    ...
    VixalValue newProposedValue(txSetHash, nextCloseTime, emptyUpgradeSteps, 0);

...
void
save(Archive &ar, vixal::Upgrades::UpgradeParameters const &p) {
    ar(make_nvp("time", vixal::VirtualClock::to_time_t(p.mUpgradeTime)));
...
template<class Archive>
void
load(Archive &ar, vixal::Upgrades::UpgradeParameters &o) {
    time_t t;
    ar(make_nvp("time", t));
    o.mUpgradeTime = vixal::VirtualClock::from_time_t(t);
...
Upgrades::createUpgradesFor(LedgerHeader const &header) const {
    ...
    if (mParams.mProtocolVersion && (header.ledgerVersion != *mParams.mProtocolVersion)) {
...
addLiabilities(std::map<Asset, std::unique_ptr<int64_t>> &liabilities,
    ...
    if (iter->second) {
        if (!vixal::addBalance(*iter->second, delta)) {
            iter->second.reset();
        }
    }
}

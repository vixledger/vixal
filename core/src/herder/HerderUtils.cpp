...
getTxSetHashes(SCPEnvelope const &envelope) {
    auto values = getVixalValues(envelope.statement);
    auto result = std::vector<Hash>{};
    result.resize(values.size());

    std::transform(std::begin(values), std::end(values), std::begin(result),
                   [](VixalValue const &sv) { return sv.txSetHash; });

    return result;
}

std::vector<VixalValue>
getVixalValues(SCPStatement const &statement) {
    auto values = Slot::getStatementValues(statement);
    auto result = std::vector<VixalValue>{};
    result.resize(values.size());

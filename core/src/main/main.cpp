...
static const struct option vixal_core_options[] = {
        ...
        {"report-last-history-checkpoint", no_argument,       nullptr, OPT_REPORT_LAST_HISTORY_CHECKPOINT},
...
static int
catchup(Config const &cfg, uint32_t to, uint32_t count, Json::Value &catchupInfo) {
    ...
    try {
        ...
        LOG(INFO) << "*";
        LOG(INFO) << "* Target ledger " << to << " is not newer than last closed ledger - nothing to do";
        LOG(INFO) << "* If you really want to catchup to " << to << " run vixal-core with --newdb parameter.";
        LOG(INFO) << "*";
        return 2;
    }
...
catchupComplete(Config const &cfg, Json::Value &catchupInfo) {
    return catchup(cfg, CatchupConfiguration::CURRENT, std::numeric_limits<uint32_t>::max(), catchupInfo);
}
    ...
    try {
        ...
        if (!checkInitialized(app)) {
            ...
            if (cfg.ARTIFICIALLY_ACCELERATE_TIME_FOR_TESTING) {
                LOG(WARNING) << "Artificial acceleration of time enabled (for testing only)";
            }
...
main(int argc, char *const *argv) {
    ...
    optional<bool> forceSCP = nullopt;
    bool base64 = false;
    ...
    int opt;
    while ((opt = getopt_long_only(argc, argv, "c:", vixal_core_options, nullptr)) != -1) {
        switch (opt) {
            case OPT_BASE64:
                base64 = true;
                break;
...
*/
            ...
            case OPT_LOGLEVEL:
                ...
                break;
            case OPT_METRIC:
                metrics.emplace_back(optarg);
                break;
            case OPT_NEWDB:
                newDB = true;
                break;
            case OPT_NEWHIST:
                newHistories.emplace_back(optarg);
                break;
 ...
 */
            case OPT_VERSION:
                std::cout << VIXAL_CORE_VERSION << std::endl;
                return 0;
            case OPT_HELP:
            default:
                usage(0);

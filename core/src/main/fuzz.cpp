 ...
 *     random VixalMessages. This is the mode you use to generate seed data
 *     for the external fuzzer's corpus.
 *
 *   - In --fuzz mode it reads back a file and applies it to a pair of of
 *     vixal-cores in loopback mode, cranking the I/O loop to simulate
...
std::string
msgSummary(VixalMessage const &m) {
...
bool
tryRead(XDRInputFileStream &in, VixalMessage &m) {
    try {
        return in.readOne(m);
    }
...
fuzz(std::string const &filename, el::Level logLevel,
    ...
    LOG(INFO) << "Fuzzing vixal-core " << VIXAL_CORE_VERSION;
    ...
    restart:
    {
        ...
        while (tryRead(in, msg)) {
            ++i;
            LOG(INFO) << "Fuzzer injecting message " << i << ": " << msgSummary(msg);
            auto peer = loop.getInitiator();
            asio::post(clock.io_context(), [peer, msg]() { peer->Peer::sendMessage(msg); });
        }
...
genfuzz(std::string const &filename) {
    ...
    out.open(filename);
    autocheck::generator<VixalMessage> gen;
    for (size_t i = 0; i < n; ++i) {
        try {
            VixalMessage m(gen(10));
            out.writeOne(m);

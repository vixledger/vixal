...
[`src/xdr/Stellar-ledger.x`](../../include/xdr/ledger.x)
...
During consensus, nodes work together to decide on the value of VixalValue.
VixalValue is then saved in the scpValue field of the ledger header.
Any node on the network, given the previous ledger (their previous state) and
'VixalValue' should be able to transition to the same new ledger.
...
[`src/xdr/Stellar-ledger-entries.x`](../../include/xdr/ledger-entries.x)

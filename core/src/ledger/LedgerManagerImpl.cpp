    ...
    switch (st) {
        ...
        case LedgerManager::LM_SYNCED_STATE:
            if (mLastClosedLedger.header.ledgerSeq + 1 == ledgerData.getLedgerSeq()) {
                if (mLastClosedLedger.hash == ledgerData.getTxSet()->previousLedgerHash()) {
                    if (st == LM_BOOTING_STATE) {
                        setState(LM_SYNCED_STATE);
                    }
                    closeLedger(ledgerData);
                    CLOG(INFO, "Ledger") << "Closed ledger: " << ledgerAbbrev(mLastClosedLedger);
                } else {
                ...
                startCatchUp({ledgerData.getLedgerSeq() - 1, getCatchupCount(mApp)}, false);
            }
            break;
...
LedgerManagerImpl::verifyCatchupCandidate(
    ...
    assert(!mSyncingLedgers.empty());
    assert(mSyncingLedgers.front().getLedgerSeq() == candidate.header.ledgerSeq + 1);
        ...
        for (auto const &lcd : mSyncingLedgers) {
            if (lcd.getLedgerSeq() < mLastClosedLedger.header.ledgerSeq + 1) {
                ...
                if (lcd.getLedgerSeq() == mLastClosedLedger.header.ledgerSeq) {
                    ...
                    assert(lcd.getTxSet()->getContentsHash() == lcd.getValue().txSetHash);
                ...
                assert(lcd.getLedgerSeq() >
                       mLastClosedLedger.header.ledgerSeq + 1);
                CLOG(ERROR, "Ledger") << "Catchup failed to buffer contiguous ledger chain";
                CLOG(ERROR, "Ledger") << "LCL is " << ledgerAbbrev(mLastClosedLedger)

...
VirtualClock::maybeSetRealtimer() {
    if (mMode == REAL_TIME) {
        auto nextp = next();
        if (nextp != timer.expiry()) {
            timer.expires_at(nextp);
            timer.async_wait([this](asio::error_code const &ec) {
...
VirtualClock::crank(bool block) {
    ...
    // calling io_context_.poll() here may introduce unbounded delays
    // to trigger timers
    const size_t WORK_BATCH_SIZE = 10;
    size_t lastPoll;
    size_t i = 0;
    do {
        lastPoll = io_context_.poll_one();
        nWorkDone += lastPoll;
...
VirtualClock::~VirtualClock() {
    mDestructing = true;
    cancelAllEvents();
    LOG(INFO) << "VirtualClock destroyed";
}
...
VirtualClockEvent::VirtualClockEvent(VirtualClock::time_point when, size_t seq,
...
VirtualClockEvent::operator<(VirtualClockEvent const &other) const {
    // For purposes of priority queue, a timer is "less than" another timer if it occurs in the future (has a higher
    // expiry time). The "greatest" timer is timer 0. To break time-based ties (but preserve the ordering in which
...
VirtualTimer::VirtualTimer(VirtualClock &clock)
        : mClock(clock), mExpiryTime(mClock.now()), mCancelled(false), mDeleting(false) {
}
...
void
VirtualTimer::expires_after(VirtualClock::duration d) {
    cancel();

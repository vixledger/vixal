...
BallotProtocol::BallotProtocol(Slot &slot)
        : mSlot(slot), mHeardFromQuorum(true),
          mPhase(SCP_PHASE_PREPARE), mCurrentMessageLevel(0) {
}
...
bool
BallotProtocol::isNewerStatement(SCPStatement const &oldst, SCPStatement const &st) {
    bool res = false;
    ...
    if (validationRes != SCPDriver::kInvalidValue) {
        ...
        if (!processed) {
            ...
            if (mPhase == SCP_PHASE_EXTERNALIZE &&
                ...
                if (self) {
                    CLOG(ERROR, "SCP") << "externalize statement with invalid value from self, skipping "
                                       << "  e: " << mSlot.getSCP().envToStr(envelope);
                }
...
BallotProtocol::isStatementSane(SCPStatement const &st, bool self) {
    ...
    switch (st.pledges.type()) {
        case SCPStatementType::SCP_ST_PREPARE: {
            ...
            isOK = isOK && (p.nH == 0 || (p.prepared && p.nH <= p.prepared->counter));
...
bool
BallotProtocol::commitPredicate(SCPBallot const &ballot, Interval const &check, SCPStatement const &st) {
    bool res = false;
...
BallotProtocol::attemptConfirmCommit(SCPStatement const &hint) {
    ...
    auto pred = [&ballot, this](Interval const &cur) -> bool {
        return federatedRatify(std::bind(&BallotProtocol::commitPredicate, ballot, cur, _1));
    };
    ...
    switch (st.pledges.type()) {
        case SCP_ST_PREPARE: {
            ...
            res = (p.prepared && areBallotsLessAndCompatible(ballot, *p.prepared)) ||
                  (p.preparedPrime && areBallotsLessAndCompatible(ballot, *p.preparedPrime));
        }
            break;
        case SCP_ST_CONFIRM: {
            auto const &c = st.pledges.confirm();
...
BallotProtocol::validateValues(SCPStatement const &st) {
    ...
    for (auto const &v : values) {
        auto tr = mSlot.getSCPDriver().validateValue(mSlot.getSlotIndex(), v, false);

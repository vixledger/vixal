...
NominationProtocol::isSane(SCPStatement const &st) {
    ...
    res = res && (std::adjacent_find(
            ...
            [](vixal::Value const &l, vixal::Value const &r) {
                return !(l < r);
            }) == nom.votes.end());
    res = res && (std::adjacent_find(
    ...
    if (isNewerStatement(st.nodeID, nom)) {
        if (isSane(st)) {
            ...
            if (mNominationStarted) {
                ...
                for (auto const &v : nom.votes) {
                    if (mAccepted.find(v) != mAccepted.end()) { // v is already accepted
                        continue;
                    }
                    if (mSlot.federatedAccept(
                            [&v](SCPStatement const &st1) -> bool {
                                auto const &nom1 = st1.pledges.nominate();
                                bool res1;
                                res1 = (std::find(nom1.votes.begin(),
                                                  nom1.votes.end(),
                                                  v) != nom1.votes.end());
                                return res1;
                            },
                ...
                if (mCandidates.empty() &&
                    ...
                    if (!newVote.empty()) {
                        ...
                        modified = true;
                        mSlot.getSCPDriver().nominatingValue(mSlot.getSlotIndex(), newVote);
                    }
                }
                ...
                if (newCandidates) {
                    mLatestCompositeCandidate =
                            mSlot.getSCPDriver().combineCandidates(mSlot.getSlotIndex(), mCandidates);
            ...
            CLOG(DEBUG, "SCP") << "NominationProtocol: message didn't pass sanity check";
        }
    }
    return res;
}

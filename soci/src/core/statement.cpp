...
void statement_impl::bind(values & values)
{
    ...
    try
    {
        for (auto it = values.uses_.begin(); it != values.uses_.end(); ++it)
        {
            ...
            if (useName.empty())
            {
                ...
                auto position = static_cast<int>(uses_.size());
            ...
            else
            {
                ...
                while (pos != std::string::npos)
                {
                    ...
                    const auto nextChar = static_cast<char>((pos + placeholder.size()) < query_.size() ?
